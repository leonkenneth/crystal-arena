...
function SelectQuickActivation({ cardOid, activations } : { cardOid : ObjectIdContainer, activations : PlayableActivationState[]}) {
    ...
    return <div>
        {activations.map((activation) => <button key={activation.abilityId} onClick={() => activateAbilityId(activation.abilityId)}>To {activation.playZone}</button>)}
    </div>
}
...
export default function Card({ card, text, size, dataCardId, containerProps, displayTextOverlay = true, displayHoverCard = card.isVisibleInUi }: Props) {
    ...
    return <CardContainer
            ...
            ref={ref}
            {...containerProps}>
        {card.isVisibleInUi ? <><Image src={imageUrl} alt={card.serial} draggable={false} />
        {displayTextOverlay && <CardBody style={{
            ...
            {playableActivations && <SelectQuickActivation cardOid={card.oid} activations={playableActivations} />}    
