...
export const ArrowPortal = ({ x1, y1, x2, y2, color = 'red', width = 2, curveHeight = 50 }: { x1: number, y1: number, x2: number, y2: number, color: string, width: number, curveHeight: number }) => {
...
export default function TargetArrow({ fromCardId, toCardId } : Props){
    ...
    const updateArrow = useCallback(function () {
    ...
    return <ArrowPortal x1={left} y1={top} x2={right} y2={bottom} color="var(--chakra-colors-blue-500)" width={2} curveHeight={50} />
}