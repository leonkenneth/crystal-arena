...
export function useDebugRerender(componentName: string, props: Record<string, unknown>) {
  ...
  useEffect(() => {
    ...
    if (Object.keys(changedProps).length > 0) {
      console.log(`[${componentName}] Render #${renderCount.current} - Changed props:`, changedProps)
    } else if (renderCount.current > 1) {
      console.log(`[${componentName}] Render #${renderCount.current} - No prop changes (parent re-rendered)`)
    } else {
