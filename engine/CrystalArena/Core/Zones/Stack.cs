namespace CrystalArena
{
    ...
    public class Stack : GameObject, IEnumerable<Effect>, IHashable, IZone
    {
        ...
        public bool IsEmpty
        {
            get { return _effects.Count == 0; }
        }

        private Effect LastResolved
        {
            get { return _lastResolved.Value; }
            set { _lastResolved.Value = value; }
        }
        public Effect TopSpell
        {
            get { return _effects.LastOrDefault(); }
        }
        public Player TopSpellOwner
        {
            get { return TopSpell == null ? null : TopSpell.Controller; }
        }
        public bool HasTriggered
        {
            get { return _triggeredEffects.Count > 0; }
        }
        ...
        public void PushTriggered()
        {
            ...
            var active = _triggeredEffects.Where(x => x.Controller == Players.Active).ToList();

            var passive = _triggeredEffects.Where(x => x.Controller == Players.Passive).ToList();

            _triggeredEffects.Clear();

            if (active.Count > 0)
            {
                Enqueue(new PushTriggeredEffects(Players.Active, active));
            }

            if (passive.Count > 0)
            {
                Enqueue(new PushTriggeredEffects(Players.Passive, passive));
            }
        }
        ...
        public int GetDamageTopSpellWillDealToForward(Card card, bool targetOnly = false)
        {
            if (TopSpell == null)
                return 0;
            ...
            var total = TopSpell.CalculateForwardDamage(card);
        ...
        public bool CanBeDealtLeathalDamageByTopSpell(Card card, bool targetOnly = false)
        {
            ...
            var dealtAmount = GetDamageTopSpellWillDealToForward(card, targetOnly);
        ...
        public bool CanBeDestroyedByTopSpell(Card card, bool targetOnly = false)
        {
            ...
            {
                return TopSpell.HasEffectTargets() ? TopSpell.HasEffectTarget(card) : !targetOnly;
            }

            if (card.Is().Forward == false)
                return false;

            return CanThougnessBeReducedToLeathalByTopSpell(card)
                || CanBeDealtLeathalDamageByTopSpell(card);
        }
