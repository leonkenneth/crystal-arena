namespace CrystalArena.AI.TargetingRules
{
    ...
    public class EffectRedirectDamageToForwardMonster : TargetingRule
    {
        protected override IEnumerable<Targets> SelectTargets(TargetingRuleParameters p)
        {
            var candidates = p.Candidates<Card>()
                .Select(x => new { Card = x, Score = CalculateDamageRedirectionScore(x, p) })
                .Where(x => x.Score > 0)
        ...
        private static int CalculateDamageRedirectionScore(Card card, TargetingRuleParameters p)
        {
            ...
            if (card.HasProtectionFrom(CardColor.Fire) || card.HasProtectionFrom(CardColor.Dark))
            {
