namespace CrystalArena.AI.TargetingRules
{
    ...
    public class EffectDealDifferentDamage : TargetingRule
    {
        ...
        protected override IEnumerable<Targets> SelectTargets(TargetingRuleParameters p)
        {
            ...
            var opponent = p.Candidates<Player>(ControlledBy.Opponent).FirstOrDefault();
            ...
            if (opponent != null)
            {
                for (var i = 0; i < _amounts.Count; i++)
                {
                    results.Add(
                        ...
                        {
                            Damage = _amounts[i],
                            Score = ScoreCalculator.CalculateLifelossScore(
                                opponent.Life,
                                _amounts[i]
                            ),
                            Target = opponent,
            ...
            var forwards = p.Candidates<Card>(ControlledBy.Opponent).ToArray();

            for (var i = 0; i < _amounts.Count; i++)
            {
                foreach (var forward in forwards.Where(x => x.Life <= _amounts[i]))
                {
                    results.Add(
                        new Result
                        {
            ...
            for (var i = 0; i < _amounts.Count; i++)
            {
                var pick = targets.FirstOrDefault(x =>
                    x.Damage <= _amounts[i] && !picks.Contains(x.Target)
                );
