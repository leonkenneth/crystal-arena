namespace CrystalArena.AI.TargetingRules
{
    ...
    public class EffectPumpSummon : TargetingRule
    {
        ...
        public EffectPumpSummon(
            Func<TargetingRuleParameters, int?> power,
            Func<TargetingRuleParameters, int?> toughness,
            bool untilEot = true
        )
        {
            _power = power;
        ...
        public EffectPumpSummon(int? power, int? toughness, bool untilEot = true)
            : this(
                delegate
                {
                    return power;
        ...
        protected override IEnumerable<Targets> SelectTargets(TargetingRuleParameters p)
        {
            ...
            if (IsBeforeYouDeclareAttackers(p.Controller))
            {
                candidates = candidates
                    .Select(x => new { Card = x.Card(), Score = CalculateAttackingPotential(x) })
                    .OrderByDescending(x => x.Score)
                    .Select(x => x.Card)
                    .ToList();
            }
            else if (IsAfterOpponentDeclaresBlocker(p.Controller))
            {
            ...
            {
                candidates = candidates.OrderBy(x => x.Toughness);
            }
            ...
            else if (toughness > 0)
            {
                candidates = candidates.Where(x =>
                    Stack.CanBeDealtLeathalDamageByTopSpell(x.Card())
                );
            }
