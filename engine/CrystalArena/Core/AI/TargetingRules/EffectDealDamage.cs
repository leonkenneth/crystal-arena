namespace CrystalArena.AI.TargetingRules
{
    ...
    public class EffectDealDamage : TargetingRule
    {
        ...
        public EffectDealDamage(int? amount = null)
            : this(p => amount ?? p.MaxX) { }
        ...
        private IEnumerable<Targets> SelectTargetsDistribute(TargetingRuleParameters p)
        {
            ...
            var targets = p.Candidates<Card>(ControlledBy.Opponent)
                .Where(x => x.Is().Forward && x.Life > 0 && x.Life <= amount)
                .Select(x => new KnapsackItem<ITarget>(item: x, weight: x.Life, value: x.Score))
                .ToList();
                            ...
                            for (var i = 1; i <= amount; i++)
                            {
                                items.Add(
                                    new KnapsackItem<ITarget>(
                                        item: x,
                                        weight: i,
                                        value: ScoreCalculator.CalculateLifelossScore(life, i)
                                            - decrementSoPlayerAtMostOnce
                                    )
                                );
                            }
                        }
            ...
            var distribution = solution.Select(x => x.Weight).ToList();
        ...
        {
            Asrt.True(
                p.EffectTargetTypeCount <= 2,
                "More than 2 effect selectors currently not supported."
            );
            ...
            return Group(
                candidates1,
                candidates2,
                minTargetCount1: p.MinTargetCount(selectorIndex: 0),
        ...
        private IList<ITarget> GetCandidatesByDescendingDamageScore(
            int damageAmount,
