namespace CrystalArena.AI.TargetingRules
{
    ...
    public class EffectPreventDamageFromSourceToTarget : TargetingRule
    {
        ...
        protected DamageSourceAndDamageTargetCandidates GetDamageSourceAndDamageTargetCandidates(
        ...
        {
            var blockerAttackerPair = p.Candidates<Card>(
                    selectorIndex: 1,
                    selector: trgs => trgs.Effect
                )
                .Where(x => x.IsAttacker)
                .Where(x => QuickCombat.CanAttackerBeDealtLeathalDamage(x, Combat.Blocker?.Card))
                .Select(x => new { Target = x, Source = Combat.Blocker?.Card })
        ...
        private void PreventDamageAttackerWillDealToBlocker(
            TargetingRuleParameters p,
            DamageSourceAndDamageTargetCandidates selectedCandidates
        )
        {
            ...
            if (blocker == null)
                return;
            var attacker = Combat
                .Attackers.Where(attacker =>
                    QuickCombat.CanBlockerBeDealtLeathalCombatDamage(attacker.Card, blocker.Card)
                )
                .FirstOrDefault();
        ...
        private void PreventDamageTopSpellWillDealToForward(
            TargetingRuleParameters p,
            DamageSourceAndDamageTargetCandidates selectedCandidates
        )
        {
            var forwardKilledByTopSpell = p.Candidates<Card>(
                    selectorIndex: 1,
                    selector: trgs => trgs.Effect
                )
        ...
        private void PreventDamageTopSpellWillDealToPlayer(
            TargetingRuleParameters p,
            DamageSourceAndDamageTargetCandidates selectedCandidates
        )
        {
