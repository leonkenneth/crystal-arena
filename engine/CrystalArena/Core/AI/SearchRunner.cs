namespace CrystalArena.AI
{
    ...
    public class SearchRunner
    {
        ...
        {
            get { return CurrentSearch != null; }
        }
        public int PlaySpellsUntilDepth
        {
            get { return _game.Turn.GetStepCountAtNextTurnCleanup(); }
        }
        ...
        public void SetBestResult(ISearchNode searchNode)
        {
            ...
            else
            {
                ...
                if (cached.ChildrenCount != searchNode.ResultCount)
                {
                    ...
                    LogFile.Debug(
                        "Invalid cached result, cached result count is {0} node's is {1}.",
                        cached.ChildrenCount,
                        searchNode.ResultCount
                    );
        ...
        private int StartNewSearch(ISearchNode searchNode, SearchResults cachedResults)
        {
            ...
            CurrentSearch = new Search(parameters, searchNode.Controller, cachedResults, _game);
