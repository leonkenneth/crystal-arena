namespace CrystalArena.AI.DraftAlgorithms
{
    ...
    public abstract class DraftingStrategy : IDraftingStrategy
    {
        ...
        protected virtual int[] ForwardsManaCurve { get; } =
            new[]
            {
                5, // up to 2
                6, // up to 3
        ...
        private Dictionary<CardColor, double> CalculateBoosterColorScores(
            IEnumerable<Card> booster,
            int round
        )
        {
            return booster
                ...
                .Select(x => new { Color = x.Key, Score = x.Max(c => GetRating(c, round)) })
                .ToDictionary(x => x.Color, x => x.Score);
        }

        protected void UpdateColorScores(
            ColorScore[] colorScores,
            Dictionary<CardColor, double> boosterScores
        )
        {
            for (var i = 0; i < colorScores.Length; i++)
            {
                if (
                    boosterScores.ContainsKey(colorScores[i].Color)
                    && boosterScores[colorScores[i].Color] > PlayableThreshold
                )
                {
        ...
        protected double GetRating(Card card, int round, bool usePlayerContext = true)
        {
            ...
            if (usePlayerContext)
            {
                ...
                {
                    if (
                        PlayableDraftedCards.Count(x => x.Is().Forward)
                        < (GiveBonusUntilForwardCountIs / (4 - round))
                    )
                    {
        ...
        protected Card GetBestCardOfChosenColor(
        ...
        {
            return scores
                .Select(x => new ColorScore { Color = x.Color, Score = x.Score })
                .ToArray();
        }

        protected Card GetBestCardOfSingleColorOrColorless(
            IEnumerable<Card> cards,
            int round,
            CardColor? primaryColor = null
        )
        {
            return cards
                .Where(x =>
                    x.Colors.Length == 1
                    || (primaryColor.HasValue && IsUsableNonbasicBackup(x, primaryColor.Value))
                )
                .OrderByDescending(c => GetRating(c, round))
                .FirstOrDefault();
        }
        ...
        protected Card GetBestCardOfChosenColorsOrColorless(
            CardColor color1,
            IEnumerable<Card> cards,
            int round,
        ...
        private bool IsUsableNonbasicBackup(Card card, CardColor color1, CardColor? color2 = null)
        {
            ...
            return backupColors.Contains((int)color1) && backupColors.Contains((int)color2.Value);
        }
        ...
        {
            var existingCurve = PlayableDraftedCards
                .Where(x => x.ManaCost != null)
                .GroupBy(GetConvertedManaCurveCost)
                .Select(x => new { Cost = x.Key, Count = x.Count() })
                .OrderBy(x => x.Cost)
                .ToArray();
            ...
            {
                var existingCount =
                    existingCurve.FirstOrDefault(x => x.Cost == (i + 2))?.Count ?? 0;

                var idealCount = idealRoundCurve[i];

                var missingCount = idealCount - existingCount;

                var bonus = missingCount > 0 ? ManaCurveForwardBonus * missingCount : 0d;
                ...
                .Where(c =>
                {
                    ...
                    if (c.Colors.Length == 1)
                    {
                        return SecondaryColor == null
                            || c.Colors[0] == PrimaryColor
                            || c.Colors[0] == SecondaryColor;
                    }
