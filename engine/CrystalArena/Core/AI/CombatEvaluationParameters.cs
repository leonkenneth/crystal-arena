namespace CrystalArena.AI
{
    ...
    public class CombatEvaluationParameters
    {
        ...
        {
            if (blocker != null)
                Blocker = new CardWithPowerIncrease()
                {
                    Card = blocker,
                    PowerIncrease = blocker.GetCombatAbilities().PowerIncrease,
                };
        ...
        public CombatEvaluationParameters(
            Card attacker,
            Card? blocker,
            int attackerPowerIncrease,
        ...
        )
        {
            if (blocker != null)
                ...
                {
                    Card = blocker,
                    PowerIncrease = blockerPowerIncrease,
                };

            AddAttacker(attacker, attackerPowerIncrease);
        }

        public CombatEvaluationParameters(Card attacker, int attackerPowerIncrease)
            : this(attacker, null, attackerPowerIncrease, 0) { }

        public CombatEvaluationParameters(Card attacker, Card? blocker)
            : this(attacker, blocker, 0, 0) { }
