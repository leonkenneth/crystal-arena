namespace CrystalArena.AI.TimingRules
{
    public class PumpTargetCardTimingRule : TimingRule
    {
        ...
        public override bool ShouldPlayBeforeTargets(TimingRuleParameters p)
        {
            ...
            {
                return (
                        Stack.TopSpell.HasTag(EffectTag.DealDamage)
            ...
            return IsBeforeYouDeclareAttackers(p.Controller)
                || IsAfterOpponentDeclaresBlocker(p.Controller)
                || IsAfterYouDeclareBlockers(p.Controller)
                || (IsEndOfOpponentsTurn(p.Controller) && !_untilEot);
        }
    }
}
