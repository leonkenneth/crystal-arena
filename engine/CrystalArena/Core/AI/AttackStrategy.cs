...
namespace CrystalArena.AI
{
    ...
    public class AttackStrategy
    {
        ...
        public List<Card> ChooseAttackers()
        {
            ...
            blockers.Add(null!);
            var assignments = parties
                .SelectMany(party =>
                {
                    return blockers.Select(blocker =>
                    {
                        return new BlockAssignment(party, blocker, _p.DefendingPlayersLife);
                    });
                })
                .ToList();
        ...
        private class BlockAssignment
        {
            ...
            private static CombatEvaluationParameters GetAttackerParamers(
                IEnumerable<CardWithCombatAbilities> attackers,
            ...
            private int CalculateAttackersScore(EvaluationParameters p)
            {
                ...
                {
                    if (
                        QuickCombat.CanAttackerBeDealtLeathalDamage(attacker.Card, p.Blocker?.Card)
                        && !attacker.Abilities.CanRegenerate
                    )
            ...
            private int CalculateBlockerScore(EvaluationParameters p)
            {
                ...
                var blockerParameters = new CombatEvaluationParameters(
                    p.Attackers.Select(x => x.Card),
                    blocker.Card
                );
                var canBeKilled = QuickCombat.CanBlockerBeDealtLeathalCombatDamage(
                    blockerParameters
                );
            ...
            private int CalculateLifelossScore(EvaluationParameters p)
            {
                ...
                return ScoreCalculator.CalculateLifelossScore(
                    p.DefendersLife,
