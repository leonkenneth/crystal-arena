...
namespace CrystalArena.AI
{
  ...
  public class QuickCombat
  {
    ...
    public static bool CanAttackerBeDealtLeathalDamageWithBoost(Card attacker, Card? blocker, int powerIncrease)
    {
      if (blocker == null) return false;
      return GetAmountOfDamageForward1WillDealToForward2(blocker, attacker, powerIncrease) >= attacker.Life;
    }

    public static int GetAmountOfDamageForward1WillDealToForward2(Card forward1, Card forward2, int? forward1PowerIncrease = null)
    {
      var powerIncrease = forward1PowerIncrease ?? forward1.GetCombatAbilities().PowerIncrease;
      var amountDealt = forward1.CalculateCombatDamageAmount(toPlayer: false, powerIncrease:powerIncrease);
      var preventedReceived = forward2.CalculatePreventedDamageAmount(amountDealt, forward1, isCombat: true);

      return amountDealt - preventedReceived;
    }
    public static PartyDamage GetAmountOfDamagePartyWillDealToForward2(Party party, Card forward2)
    {
      var damage = new PartyDamage();
      foreach (var attacker in party.Attackers)
      {
        damage.Add(attacker, GetAmountOfDamageForward1WillDealToForward2(attacker, forward2));
      }

      return damage;
    }

    public static int GetAmountOfDamageThatNeedsToBePreventedToSafeBlockerFromDying(Card blocker, Party party)
    {
      var evaluation = new BlockerEvaluation(new CombatEvaluationParameters(party.Attackers, blocker));
    ...
    public static int GetAmountOfDamageThatNeedsToBePreventedToSafeAttackerFromDying(Party party, Card attacker,
      Card blocker)
    {
      ...
      if (party.HasFirstStrike && !blocker.Has().FirstStrike && attackerDamage.Total > blockerLife)
        return 0;
    ...
    public static PartyDamage CalculateDefendingPlayerLifeloss(CombatEvaluationParameters p)
    {
      ...
      if (p.Blocker == null)
      {
        ...
        {
          var dealt = a.Card.CalculateCombatDamageAmount(toPlayer: true, singleDamageStep: false,
            powerIncrease: a.PowerIncrease);
          var prevented = a.Card.Controller.Opponent.CalculatePreventedReceivedDamageAmount(dealt, a.Card,
            isCombat: true);
      ...
      Card? blocker, int powerIncrease, int toughnessIncrease)
    {
      ...
      var canBeDealtLeathalDamageWithoutBoost = CanAttackerBeDealtLeathalDamage(attacker, blocker);

      if (canBeDealtLeathalDamageWithoutBoost == false)
        return 0;

      var canBeDealtLeathalDamageWithBoost = CanAttackerBeDealtLeathalDamageWithBoost(attacker, blocker, powerIncrease);

      return canBeDealtLeathalDamageWithBoost ? 0 : attacker.Score;
    }

    public static int CalculateGainBlockerWouldGetIfPowerAndThougnessWouldIncrease(Party party,
      Card blocker, int powerIncrease, int toughnessIncrease)
    {
      var p = new CombatEvaluationParameters(party.Attackers, blocker);
    ...
    public static bool CanBlockerBeDealtLeathalCombatDamage(Card attacker, Card? blocker)
    {
      if (blocker == null) return false;
      return
        CanBlockerBeDealtLeathalCombatDamage(new CombatEvaluationParameters(attacker, blocker));
    }
    ...
    public static bool CanBlockerBeDealtLeathalCombatDamage(Party party, Card? blocker)
    {
      if (blocker == null) return false;
      return
        CanBlockerBeDealtLeathalCombatDamage(new CombatEvaluationParameters(party.Attackers, blocker));
    }
