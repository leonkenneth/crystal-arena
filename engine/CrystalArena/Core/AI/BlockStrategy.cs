namespace CrystalArena.AI
{
  ...
  public class BlockStrategy
  {
    public static ChosenBlocker ChooseBlocker(BlockStrategyParameters p)
    {
      ...
      {
        if (!party.CanBeBlockedBy(blocker)) continue;

        candidates.Add(new BlockerCandidate(party, blocker, p.DefendersLife));
      }

      var chosenCandidate = candidates.Where(x => x.Gain > 0).OrderByDescending(x => x.Gain).FirstOrDefault();
    ...
    private class BlockerCandidate
    {
      ...
      private void ComputeGain(Party party, Card blocker, int defendersLife)
      {
        ...
        var blockerScore = canBlockerBeDealtLeathalCombatDamage
          ? blocker.Score
          : 0;

        var lossIfNotBlocked =
          QuickCombat.CalculateDefendingPlayerLifeloss(new CombatEvaluationParameters(party.Attackers, null));
        ...
        {
          var isAttackerKilled = QuickCombat.CanAttackerBeDealtLeathalDamage(attacker, blocker);
          return isAttackerKilled
            ? attacker.Score
            : 0;
        });
