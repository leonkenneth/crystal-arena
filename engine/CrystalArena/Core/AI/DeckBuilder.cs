namespace CrystalArena.AI
{
    ...
    public static class DeckBuilder
    {
        ...
        {
            var cards = library.Select(x => Cards.All[x.Name]).ToList();
        ...
        private static Deck CreateDeckFromCardList(List<Card> cards, IEnumerable<CardInfo> library)
        {
            ...
            {
                var info =
                    library.FirstOrDefault(x => x.Name.Equals(card.Name))
                ...
                for (var j = i + 1; j <= CardColor.Wind; j++)
                {
                    var dual = cards
                        .Where(x =>
                            x.HasColor(i)
                            || x.HasColor(j)
                            || x.HasColor(CardColor.Colorless)
                            || x.Is().Backup
                        )
                        .ToList();
        ...
        private static List<Card> AddBackups(
            List<Card> deck,
            List<Card> library,
            CardRatings cardRatings,
            int backupCount
        ...
        private static void AddNonBasicBackups(
        ...
        private static List<int> GetBackupsDistribution(
            IEnumerable<Card> noBackupsDeck,
            int backupCount
        )
        {
            ...
            Asrt.True(
                distribution.Sum() == backupCount,
                "Backup distribution sum should equal backup count."
            );
            return distribution;
        }
        ...
        private static DeckCandidate BuildDeck(List<Card> cards, CardRatings cardRatings)
        {
            ...
            var forwards = cards
                ...
                .Select(x =>
                {
                    ...
                    var ordered = x.OrderByDescending(y => y.Rating).ToList();
                    ...
                    return new { Selected = selected, Skipped = skipped };
                })
                .ToList();

            var selectedForwards = forwards.SelectMany(x => x.Selected).ToList();
            ...
            var selectedRemaining = remaining.Take(SpellCount - selectedForwards.Count).ToList();

            var avarageRating = selectedForwards.Concat(selectedRemaining).Average(x => x.Rating);
