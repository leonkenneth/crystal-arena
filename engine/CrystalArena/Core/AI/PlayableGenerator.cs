namespace CrystalArena.AI
{
    ...
    public class PlayableGenerator : GameObject
    {
        ...
        )
        {
            ...
            foreach (
                var targetsCombination in context.TargetsCombinations().Take(Ai.CurrentTargetCount)
            )
            {
                var playable = createPlayable();
        ...
        public List<IPlayable> GetPlayables()
        {
            ...
            FindPlayableAbilities(searchSpace: _player.BreakZone, result: result);

            FindPlayableAbilities(searchSpace: _player.Hand, result: result);

            return result;
        }

        public List<IPlayable> GetPlayablesForSpell(
            Card card,
        ...
        )
        {
            ...
            IEnumerable<ActivationPrerequisites> spellsPrerequisites = card.GetCastPrerequisites(
                    payManaCost
                )
            ...
            foreach (var prerequisites in spellsPrerequisites)
            {
                var playables = GeneratePlayables(
                    prerequisites,
                    () => new PlayableSpell { ActivationParameters = { PayManaCost = payManaCost } }
                );
