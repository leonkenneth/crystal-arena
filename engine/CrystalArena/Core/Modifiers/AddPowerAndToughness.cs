namespace CrystalArena.Modifiers
{
    ...
    public class AddPowerAndToughness : Modifier, ICardModifier
    {
        ...
        public override void Apply(Strength strength)
        {
            ...
            _powerIntegerIncrement = new IntegerIncrement(
                _power(OwningCard.Controller).GetValue(X)
            );
            _powerIntegerIncrement.Initialize(ChangeTracker);
            _strength.AddPowerModifier(_powerIntegerIncrement);

            _toughnessIntegerIncrement = new IntegerIncrement(
                _toughness(OwningCard.Controller).GetValue(X)
            );
