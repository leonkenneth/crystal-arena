namespace CrystalArena.Modifiers
{
    ...
    public class Add11ForEachOtherForward
        : Modifier,
            ICardModifier,
            IReceive<ZoneChangedEvent>,
            IReceive<TypeChangedEvent>,
            IReceive<ControllerChangedEvent>
    {
        ...
        {
            if (message.Card.Controller != SourceCard.Controller || !message.Card.Is().Forward)
            {
                return;
            }
        ...
        {
            return SourceCard.Controller.Battlefield.Count(card =>
                card != Owner && card.Is().Forward
            );
        }
    }
}
