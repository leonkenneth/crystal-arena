...
namespace CrystalArena
{
  ...
  public static class Cards
  {
    ...
    public static List<string> ListSerials() { return Templates.Where(x => x.Serial != null).Select(x => x.Serial).ToList(); }

    private static CardTemplate GetCardTemplate(string nameOrSerial)
    {
      var template = LooksLikeFFTCGSerial(nameOrSerial) ? FindCardTemplateBySerial(nameOrSerial) : FindCardTemplateByName(nameOrSerial);
    ...
    {
      return Templates
        .FirstOrDefault(x => x.Serial != null ? x.Serial.Equals(serial, StringComparison.InvariantCultureIgnoreCase) : false);
    }
