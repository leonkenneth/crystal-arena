namespace CrystalArena
{
    ...
    public class TriggeredAbility : Ability, ICopyContributor, IHashable
    {
        ...
        public TriggeredAbility(Parameters p)
            : base(p)
        {
            _p = p;
        }
        ...
        {
            return calc.Calculate(_p.Triggers, orderImpactsHashcode: false);
        }

        public override void Initialize(Card owningCard, Game game)
        {
            base.Initialize(owningCard, game);
            _lastTrigger.Initialize(ChangeTracker);
        ...
        public virtual void Execute(object triggerMessage)
        {
            ...
            if (effect is ModalEffect)
            {
                _originalTriggerMessage = triggerMessage;
                Enqueue(
                    new ChoseModalEffect(
                        OwningCard.Controller,
                        p =>
                        {
                            p.Effect = (effect as ModalEffect);
                            p.TriggeredAbility = this;
                        }
                    )
                );
                return;
            ...
            if (_p.TriggerOnlyOncePerTurn && _lastTrigger.Value == Turn.TurnCount)
            {
                return;
            }
            _lastTrigger.Value = Turn.TurnCount;

            if (parameters.TargetSelector.Count > 0)
            {
                Enqueue(
                    new SetTriggeredAbilityTarget(
                        OwningCard.Controller,
                        p =>
                        {
                            p.Effect = effect;
                            p.TargetSelector = parameters.TargetSelector;
        ...
        public class Parameters : AbilityParameters
        {
            ...
            public bool IsExBurst { get; set; }
            public bool TriggerOnlyOncePerTurn { get; set; }
        ...
        public void RunSelectedModalEffectParameters(Action<AbilityParameters> setParams)
        {
            setParams(_p);
            Execute(_originalTriggerMessage!);
        }
    }
}
