...
namespace CrystalArena
{
    ...
    {
        private readonly StandardAnalyzer _analyzer = new StandardAnalyzer(LuceneVersion.LUCENE_30);
        private readonly Dictionary<string, Card> _cards = Cards.All.ToDictionary(
        ...
        public IEnumerable<Card> Query(string searchStr = null)
        {
            ...
            var directoryIndexReader = DirectoryReader.Open(_directory);
            var searcher = new IndexSearcher(directoryIndexReader);
        ...
        {
            var indexWriterConfig = new IndexWriterConfig(LuceneVersion.LUCENE_30, _analyzer);
            using (var writer = new IndexWriter(_directory, indexWriterConfig))
            {
                foreach (var card in Cards.All)
                {
                    ...
                    document.Add(
                        new Field(
                            "text",
                            card.Text.GetTextOnly(),
                            Field.Store.YES,
                            Field.Index.ANALYZED
                        )
                    );
                    document.Add(
                        new Field("name", card.Name, Field.Store.YES, Field.Index.ANALYZED)
                    );
                    document.Add(
                        new Field(
                            "type",
                    ...
                    document.Add(
                        new Field(
                            "power",
                            card.Power.GetValueOrDefault().ToString(),
                            Field.Store.YES,
                    ...
                    document.Add(
                        new Field(
                            "toughness",
                            card.Toughness.GetValueOrDefault().ToString(),
                            Field.Store.YES,
                            Field.Index.NOT_ANALYZED
                        )
                    );
        ...
        {
            var parser = new QueryParser(LuceneVersion.LUCENE_30, "name", _analyzer);
