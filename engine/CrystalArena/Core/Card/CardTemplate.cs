namespace CrystalArena
{
  ...
  public class CardTemplate
  {
    ...
    {
      if (cp.Colors == null || cp.Colors.Contains(CardColor.Dark) || cp.Colors.Contains(CardColor.Light)) return;
    ...
    public CardTemplate ContinuousEffect(Action<ContinuousEffectParameters> set, bool enabledInAllZones = false)
    {
      _init.Add(cp =>
      {
    ...
    public CardTemplate Exalted()
    {
       TriggeredAbility(p =>
        {
          p.Text = "Exalted{I}(Whenever a forward you control attacks alone, that forward gets +1/+1 until end of turn.){/I}";
    ...
    public CardTemplate Outlast(string cost)
    {
      ...
      {
        p.Text =
          string.Format("Outlast {0}({0}, {{T}}: Put a +1/+1 counter on this forward. Outlast only as a sorcery.)",
            cost);
    ...
    public CardTemplate Prowess()
    {
      TriggeredAbility(p =>
      {
        p.Text = "Whenever you cast a nonforward spell, this forward gets +1/+1 until end of turn.";
    ...
    public CardTemplate Cost(int cp, string colors)
    {
      ...
      if (leftover > 0) costString = $"{{{leftover}}}{costString}";
      return ManaCost(costString);
    }

    public CardTemplate SpecialAbility(string abilityName, Action<ActivatedAbilityParameters> init, Cost? additionalCost = null)
    {
      var cardName = Name;
      return ActivatedAbility(p =>
      {
