namespace CrystalArena
{
    ...
    public class ManaAbility : ActivatedAbility, IManaSource, ICopyContributor
    {
        ...
        bool IManaSource.CanActivate()
        {
            ...
            using (_guard.Set())
            {
                return _isEnabled && CanPay().CanPay && CanBeActivatedAtThisTime();
            }
        }
        ...
        void IManaSource.PayActivationCost()
        {
            ...
            if (_p.AdditionalEffects.Count > 0)
            {
                ...
                var effectParameters = new EffectParameters { Source = this };
        ...
        protected override Effect CreateEffect()
        {
            ...
            var effects = addManaToPool.Concat(_p.AdditionalEffects.Select(x => x())).ToArray();
        ...
        {
            if (OwningCard != null && OwningCard.Zone == _p.ActivationZone)
            {
                ActivateSource();
            }
            base.OnEnable();
        ...
        private void SubscribeToEvents()
        {
            ...
            if (_p.ActivationZone == Zone.Battlefield)
            {
                OwningCard.JoinedBattlefield += ActivateSource;
                OwningCard.LeftBattlefield += DeactivateSource;
            }
            else if (_p.ActivationZone == Zone.Hand)
            {
                OwningCard.JoinedHand += ActivateSource;
                OwningCard.LeftHand += DeactivateSource;
            }
            else
            {
                throw new ArgumentException(
                    "Mana ability can only be activated from battlefield or hand."
                );
            }
        }
        ...
        {
            return new ManaUnit(color, _p.Priority, this, _p.UsageRestriction);
        }
    }
}
