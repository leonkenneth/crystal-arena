    ...
    public class ActivatedAbility : Ability, IHashable
    {
        ...
        public void Activate(ActivationParameters p)
        {
            ...
            for (var i = 0; i < p.Repeat; i++)
            {
                ...
                // put owning card to breakZone which will alter some card
        ...
        public override void Initialize(Card owningCard, Game game)
        {
            ...
            _p.Cost.Initialize(
                ...
                owningCard,
                game,
                _p.TargetSelector.Cost.FirstOrDefault()
            );
        ...
        protected bool CanBeActivatedAtThisTime()
        {
            ...
            if (_p.ActivateOnlyDuringYourTurn && !Game.Players.Active.Equals(SourceCard.Controller))
            {
                return false;
            }
            ...
            if (
                OwningCard.Is().Planeswalker
                && Turn.Events.HasAnyLoyalityAbilityBeenActivated(OwningCard)
            )
            {
                return false;
            }
            ...
            {
                return Turn.Step.IsMain() && OwningCard.Controller.IsActive && Stack.IsEmpty;
            }
