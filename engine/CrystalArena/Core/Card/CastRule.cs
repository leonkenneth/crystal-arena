namespace CrystalArena
{
    ...
    public class CastRule : GameObject, IEffectSource
    {
        ...
        {
            _card.ChangeZone(destination: Stack, add: delegate { });
        }
        ...
        public bool ValidateTargetDependencies(
            List<ITarget> costTargets,
            List<ITarget> effectTargets
        )
        {
            return _p.TargetSelector.ValidateTargetDependencies(
                new ValidateTargetDependenciesParam { Cost = costTargets, Effect = effectTargets }
            );
        }

        private void PutToZoneAfterResolve(Card card, Effect.Context ctx)
        {
            if (card.Is().Sorcery || card.Is().Summon)
            {
                card.PutToBreakZone();
                return;
            }
        ...
        private bool CanBeCastAtThisTime()
        {
            ...
            {
                return _card.Zone == Zone.LimitBreak
                    && _card.Controller.IsActive
                    && Turn.Step.IsMain()
                    && Stack.IsEmpty;
            }
        ...
        {
            return !_card.Is().Backup || _card.Controller.CanPlayBackups;
        }
        ...
        public void Cast(ActivationParameters p)
        {
            ...
            if (_card.Is().Forward || _card.Is().Monster || p.SkipStack)
            {
                effect.QuickResolve();
                return;
            }
            ...
            if (_card.Is().Backup)
            {
                _card.Controller.BackupsPlayedCount++;

                effect.QuickResolve();
                Publish(new BackupPlayedEvent(effect.Source.OwningCard));
