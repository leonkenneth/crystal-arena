namespace CrystalArena.Triggers
{
    ...
    public class OnBeingTargetedBySpellOrAbility : Trigger, IReceive<EffectPutOnStackEvent>
    {
        ...
        public OnBeingTargetedBySpellOrAbility(
            Func<ITarget, Effect, Trigger, bool> predicate = null
        )
        {
            _predicate =
                predicate ?? ((target, effect, trigger) => target == trigger.Ability.OwningCard);
        }

        public void Receive(EffectPutOnStackEvent message)
        {
            var effectTargets = message
                .Effect.Targets.Where(target => _predicate(target, message.Effect, this))
                .ToList();
