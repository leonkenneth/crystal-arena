namespace CrystalArena.Effects
{
    ...
    public class ForwardsOfChosenTypeCostLess : CustomizableEffect
    {
        private readonly int _amount;
        private string _mostCommonType;

        private ForwardsOfChosenTypeCostLess() { }

        public ForwardsOfChosenTypeCostLess(int amount)
        {
            _amount = amount;
        }
        ...
        public override void ProcessResults(ChosenOptions results)
        {
            ...
            var costModifier = new ChangeManaCostOfSpellsOrAbilities(
                -_amount,
                (card, usage, self) =>
                    usage == CostType.Spell && card.Is().Forward && card.Is(chosenType)
            );

            var addCostModifier = new AddCostModifier(costModifier);

            addCostModifier.AddLifetime(
                new PermanentLeavesBattlefieldLifetime(self => self.Modifier.SourceCard)
            );
        ...
        public override IEnumerable<IEffectChoice> GetChoices()
        {
            ...
            var cards = Controller.MainDeck.Concat(Controller.Hand).Where(x => x.Is().Forward);
