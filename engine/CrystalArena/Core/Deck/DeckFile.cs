namespace CrystalArena
{
    ...
    public class DeckFile
    {
        ...
        private static readonly Regex LimitedCodeRegex = new Regex(
            @"#.*Limited\:\s*(.+)",
            RegexOptions.Compiled
        );

        private static readonly Regex DeckRowRegex = new Regex(
            @"([0-9]+)\s+([^()]+)\s*(?:\((.)\,\s+(.+)\))*",
            RegexOptions.Compiled
        );
        ...
        private static DeckRow ParseRow(string line, int lineNumber)
        {
            ...
            Asrt.True(match.Success, String.Format("Error parsing line {0}.", lineNumber));
