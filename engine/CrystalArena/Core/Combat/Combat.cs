...
namespace CrystalArena
{
    ...
    public class Combat : GameObject, IHashable
    {
        ...
        private readonly TrackableList<AssignedDamage> _assignedDamage =
            new TrackableList<AssignedDamage>();
        ...
        {
            get { return _blocker.Value == null ? 0 : 1; }
        }

        private Player DefendingPlayer
        {
            get { return Players.Defending; }
        }
        ...
        public void DistributeFirstStrikeCombatDamage()
        {
            ...
            if (blocker.HasFirstStrike)
            {
                foreach (var assignment in _blockerDamageAssignment.AttackerDamageAssignments)
                {
                    ...
                    {
                        _assignedDamage.Add(
                            new AssignedDamage(assignment.Value, blocker.Card, assignment.Key.Card)
                        );
                    }
                }
            }
            ...
            {
                var damageToBlocker = QuickCombat.GetAmountOfDamagePartyWillDealToForward2(
                    party,
                    forward2: blocker.Card
                );
                _assignedDamage.Add(
                    new AssignedDamage(amount: damageToBlocker, source: party, target: blocker.Card)
                );
            }
        }
        ...
        {
            Enqueue(new SetDamageAssignments(Players.Defending, _attackers, _blocker));
        }
        ...
        public Card FindAttackerWhichWillDealGreatestDamageToDefendingPlayer(
