namespace CrystalArena
{
    ...
    {
        private readonly Trackable<IDecisionHandler> _currentHandler =
            new Trackable<IDecisionHandler>();
        ...
        private IDecisionHandler CurrentHandler
        {
            get { return _currentHandler.Value; }
            set { _currentHandler.Value = value; }
        }
        public bool WasPriorityPassed
        {
            get { return CurrentHandler.IsPass; }
        }
        public bool HasPendingDecisions
        {
            get { return _decisionQueue.Count > 0; }
        }
        ...
        {
            Enqueue(new DeclareAttackers(Players.Attacking));
        }

        private void DeclareBlocker()
        {
            Enqueue(new DeclareBlocker(Players.Defending));
        }
        ...
        private void DiscardToMaximumHandSize()
        {
            ...
            Enqueue(new DiscardCards(Players.Active, p => p.Count = discardCount));
        }
        ...
        private List<StepNode> CreateStepNodes()
        {
            ...
            gameStart = NewStep(
                Step.GameStart,
                new Action[] { SelectStartingPlayer, ShuffleMainDecks, DrawStartingHands },
                next: () => mulligan,
                getsPriority: false
            );
                ...
                new Action[]
                {
                    ...
                    () =>
                    {
                        foreach (var permanent in Players.Active.Battlefield)
                        {
                            ...
                            {
                                Enqueue(new ChooseToUntap(Players.Active, permanent));
                            }
                            else
                            {
                                permanent.Untap();
                        ...
                        Players.Active.BackupsPlayedCount = 0;
                    },
                },
                next: () => upkeep,
                getsPriority: false
            ...
            upkeep = NewStep(Step.Upkeep, next: () => draw, getsPriority: true);
            ...
            firstMain = NewStep(Step.FirstMain, next: () => beginOfCombat, getsPriority: true);

            beginOfCombat = NewStep(
                Step.BeginningOfCombat,
                next: () => declareAttackers,
                getsPriority: true
            );

            declareAttackers = NewStep(
                Step.DeclareAttackers,
                new Action[] { DeclareAttackers },
                next: () => Combat.Attackers.Any() ? declareBlockers : secondMain,
                getsPriority: true
            );

            declareBlockers = NewStep(
                Step.DeclareBlocker,
                new Action[] { DeclareBlocker, SetDamageAssignments },
                next: () =>
                    Combat.AnyForwardsWithFirstStrike() ? firstStrikeCombatDamage : combatDamage,
                getsPriority: true
            );

            firstStrikeCombatDamage = NewStep(
                Step.FirstStrikeCombatDamage,
                new Action[]
                {
                    () => Combat.DistributeCombatDamage(firstStrike: true),
                    DealAssignedCombatDamage,
                    () => CheckAndMovePermanentsToBreakZone(),
                },
                next: () => Combat.AnyForwardsWithNormalStrike() ? combatDamage : endofCombat,
                getsPriority: true
            );

            combatDamage = NewStep(
                Step.CombatDamage,
                new Action[]
                {
            ...
            endofCombat = NewStep(
                Step.EndOfCombat,
                new Action[] { () => Combat.RemoveAll() },
                next: () => beginOfCombat,
                getsPriority: true
            );

            secondMain = NewStep(Step.SecondMain, next: () => endOfTurn, getsPriority: true);

            endOfTurn = NewStep(Step.EndOfTurn, next: () => cleanup, getsPriority: true);
        ...
        private StepNode NewStep(
            Step step,
            IEnumerable<Action> before,
            bool getsPriority,
            Func<StepNode> next
        )
        {
        ...
        private void CheckAndMovePermanentsToBreakZone()
        {
            MovePermanentsToBreakZone(Players.Active);
            MovePermanentsToBreakZone(Players.Passive);

            CheckLegendaryRule(Players.Active);
            CheckLegendaryRule(Players.Passive);
        ...
        private void CheckPlaneswalkerRule(Player player)
        {
             ...
             * If we want strict ordering we should implement
               ...
               then move to breakZone in active, passive order
            */

            var duplicatePlaneswalkers = player
                .Battlefield.Where(x => x.Is().Planeswalker)
                ...
                Enqueue(
                    new SelectCards(
                        ...
                        p =>
                        {
                            ...
                            p.ValidCards = new List<Card>
                            {
                                duplicatePlaneswalkers[i],
                                duplicatePlaneswalkers[i + 1],
                            };
                            p.SetValidator(c => p.ValidCards.Contains(c));
                            p.Text = String.Format(
                                "{0}: Choose a planeswalker to sacrifice.",
                                duplicatePlaneswalkers[i].Name
                            );

                            p.SetChooseDecisionResults(candidates => new ChosenCards(
                                candidates.OrderBy(x => x.Loyality).First()
                            ));

                            p.SetProcessDecisionResults(results => results[0].Sacrifice());
                        }
                    )
                );
            }
        ...
        private void CheckLegendaryRule(Player player)
        {
            var duplicateLegends = player
                .Battlefield.Legends.GetDuplicates(card => card.Name)
                .ToArray();
        ...
        private void MovePermanentsToBreakZone(Player player)
        {
        ...
        private void DealAssignedCombatDamage()
        {
            ...
            if (Combat.AnyForwardsWithFirstStrike() && Turn.Step == Step.FirstStrikeCombatDamage)
                Publish(new AssignedDamageDealtEvent(Turn.Step));

            if (Combat.AnyForwardsWithNormalStrike() && Turn.Step == Step.CombatDamage)
                Publish(new AssignedDamageDealtEvent(Turn.Step));
        }
        ...
        private Player RollDice()
        {
            ...
            return result1 > result2 ? Players.Player1 : Players.Player2;
        }
        ...
        private void SetDamageAssignments()
        {
            Combat.SetDamageAssignments();
        }

        private void ShuffleMainDecks()
        {
            foreach (var player in Players)
            {
            ...
            if (getsPriority)
            {
                ...
                priorityPassive = new StateNode(
                    ...
                    () =>
                    {
                        if (WasPriorityPassed)
                        {
                            return Stack.IsEmpty ? priorityAfter : priorityBeginResolve;
                        }
                ...
                priorityFinishResolve = new StateNode(
                    ...
                    () =>
                    {
                        ...
                        // e.g after forward type for engineered plague has been
                        // selected
