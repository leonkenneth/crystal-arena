namespace CrystalArena.Decisions
{
  ...
  {
    private readonly Dictionary<Attacker, int> _attackerDamageAssigments = new Dictionary<Attacker, int>();
    ...
    protected DamageAssignment(SerializationInfo info, StreamingContext context)
    {
      ...
      var forwardDamageAssignments = (List<ForwardDamageAssignment>) info.GetValue("forwardDamageAssignments", typeof (List<ForwardDamageAssignment>));

      foreach (var forwardDamageAssignment in forwardDamageAssignments)
      {
        var attacker = ctx.Game.Combat.FindAttacker((Card) ctx.Recorder.GetObject(forwardDamageAssignment.CardId));
    ...
    {
      var forwardDamageAssignments = _attackerDamageAssigments.Select(x => new ForwardDamageAssignment()
        {
