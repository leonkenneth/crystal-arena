namespace CrystalArena.Decisions
{
    ...
    public class PlaySpellOrAbility : Decision
    {
        ...
        private class MachineHandler : Handler, ISearchNode, IMachineExecutionPlan
        {
            ...
            {
                if (
                    Stack.TopSpellOwner == Controller
                    || (Ai.IsSearchInProgress && Turn.StepCount > Ai.PlaySpellsUntilDepth)
                )
                {
        ...
        private class UiHandler : Handler
        {
            ...
            {
                if (
                    Ui.Configuration.ShouldAutoPass(
                        step: Game.Turn.Step,
                        isActiveTurn: D.Controller.IsActive,
                        anyPlayerPlayedSomething: Game.Turn
                            .Events
                            .HasAnythingBeenPlayedOrActivatedDuringThisStep
                    )
                )
                {
                    Result = new ChosenPlayable { Playable = new Pass() };
                ...
                Ui.Shell.ShowModalDialog(
                    dialog,
                    DialogType.Small,
                    InteractionState.PlaySpellsOrAbilities
                );
