namespace CrystalArena.Decisions
{
    ...
    public class DiscardCards : Decision
    {
        ...
        private abstract class Handler : DecisionHandler<DiscardCards, ChosenCards>
        {
            ...
            {
                get { return D._p.DiscardOpponentsCards ? D.Controller.Opponent : D.Controller; }
            }
        ...
        private class MachineHandler : Handler
        {
            ...
            {
                var cardsToDiscard = CardsOwner
                    .Hand.Where(D._p.Filter)
                    .Select(card => new
                    {
                        Card = card,
                        Score = ScoreCalculator.CalculateDiscardScore(
                            card,
                ...
                Result = cardsToDiscard.Take(D._p.Count).Select(x => x.Card).ToList();
            }
        }
        ...
        public class Params
        {
            ...
            public Func<Card, bool> Filter = delegate
            {
                return true;
            };
        }
        ...
        private class UiHandler : Handler
        {
            protected override void ExecuteQuery()
            {
                ...
                var dialog = Ui.Dialogs.SelectTarget.Create(
                    new SelectTargetParameters { CanCancel = false, Validator = targetValidator }
                );
