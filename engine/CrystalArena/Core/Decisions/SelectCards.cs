namespace CrystalArena.Decisions
{
    ...
    public class SelectCards : Decision
    {
        ...
        private abstract class Handler : DecisionHandler<SelectCards, ChosenCards>
        {
            ...
            protected List<Card> ValidTargets
            {
                get
                {
                    return _validCards
                        ?? (
                            _validCards = GenerateTargets(
                                    (zone, owner) =>
                                    {
                                        if (
                                            D._p.CanSelectOnlyCardsControlledByDecisionController
                                            && owner != D.Controller
                                        )
                                            return false;
        ...
        public class Params
        {
            ...
            {
                ChooseDecisionResults = new ChooseDecisionResultsHelper<List<Card>, ChosenCards>(
                    chooseResults
                );
            }
            ...
            {
                ProcessDecisionResults = new ProcessDecisionResultsHelper<ChosenCards>(
                    processResults
                );
            }
        }
