namespace CrystalArena.Decisions
{
    ...
    public class PushTriggeredEffects : Decision
    {
        ...
        private abstract class Handler : DecisionHandler<PushTriggeredEffects, Ordering>
        {
            ...
            {
                var effects = D._effects.ToList().ShuffleInPlace(Result.Indices);
        ...
        private class MachineHandler : Handler
        {
            ...
            {
                var ordered = D
                    ._effects.Select((x, i) => new { Effect = x, Index = i })
                    .OrderBy(x => x.Effect.TriggerOrderRule)
                    .Select(x => x.Index)
                    .ToList();

                var indices = Enumerable.Repeat(0, ordered.Count).ToArray();
        ...
        private class UiHandler : Handler
        {
            protected override void ExecuteQuery()
            {
                var dialog = Ui.Dialogs.CardOrder.Create(
                    D._effects.Select(x => x.Source),
                    "Set order, lower gets pushed first"
                );
