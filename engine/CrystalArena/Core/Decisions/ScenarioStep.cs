namespace CrystalArena.Decisions
{
    ...
    public class ScenarioStep
    {
        ...
        public ScenarioStep Activate(
            Card card,
            Card target,
            Card costTarget = null,
            int? x = null,
            int abilityIndex = 0,
            bool stackShouldBeEmpty = true
        )
        {
        ...
        public ScenarioStep Activate(Action<ScenarioActivation> init)
        {
            ...
            _results.Add(
                new DecisionResult(
                    result,
                    activation.Card.Controller,
                    activation.StackShouldBeEmpty
                )
            );
            return this;
        }
        ...
        public void AssertAllWereExecuted()
        {
            Asrt.True(
                _results.Count == 0,
                String.Format(
                    "All decisions of step {0} of turn {1} were not executed!",
                    Step,
                    Turn
                )
            );
        ...
        public ScenarioStep DeclareBlockers(params Card[] pairs)
        {
            Debug.Assert(
                pairs.Length % 2 == 0,
                "Pairs lenght must be even number, did you forget to match a blocker?"
            );
            var chosenBlockers = new ChosenBlocker();
            ...
            for (var i = 0; i < pairs.Length; i = i + 2)
            {
                ...
                chosenBlockers.Blocker = blocker;
            }
        ...
        public TResult PopNext<TResult>(Player controller)
            where TResult : class
        {
            if (_results.Count == 0)
            {
        ...
        private ScenarioStep Activate(
            Card card,
            ITarget target = null,
            ITarget costTarget = null,
            int? x = null,
            int abilityIndex = 0,
            bool stackShouldBeEmpty = true
        )
