namespace CrystalArena.Decisions
{
    ...
    public class DeclareAttackers : Decision
    {
        ...
        {
            private List<Card> _forwardsThatMustAttack;
            private List<Card> _planeswalkers;

            protected List<Card> ForwardsThatMustAttack
            {
                get
                {
                    return _forwardsThatMustAttack
                        ?? (
                            _forwardsThatMustAttack = D
                                .Controller.Battlefield.ForwardsThatCanAttack.Where(x =>
                                    x.Has().AttacksEachTurnIfAble && x.CombatCost == 0
                                )
                                .ToList()
                        );
                }
            ...
            protected List<Card> Planeswalkers
            {
                get
                {
                    return _planeswalkers
                        ?? (
                            _planeswalkers = D
                                .Controller.Opponent.Battlefield.Where(x => x.Is().Planeswalker)
                                .OrderBy(x => -x.Score)
                                .ToList()
                        );
                }
            ...
            {
                get { return D.Controller.Battlefield.HasForwardsThatCanAttack; }
            }
            ...
            public override void ProcessResults()
            {
                foreach (var attacker in Result)
                {
                    ...
                    if (
                        ...
                        && (combatCost == 0 || D.Controller.HasMana(combatCost))
                    )
                    {
                        ...
                        Combat.AddAttacker(attacker.Card);
                    }
                }
        ...
        private class MachineHandler : Handler, ISearchNode, IMachineExecutionPlan
        {
            ...
            private List<ChosenAttackers> GetAttackersDeclarations()
            {
                ...
                results.Add(SelectAttackTarget(ForwardsThatMustAttack));

                var everyForwardThatCanAttack = RemoveAttackersIfWeCannotAffordToPayCombatCost(
                    Controller.Battlefield.ForwardsThatCanAttack.ToList()
                );

                if (everyForwardThatCanAttack.Count > ForwardsThatMustAttack.Count)
                {
                    var parameters = new AttackStrategyParameters
                    {
                        AttackerCandidates = everyForwardThatCanAttack,
                        BlockerCandidates = Defender.Battlefield.ForwardsThatCanBlock.ToList(),
                        DefendingPlayersLife = Defender.Life,
                    };

                    var greedyAttackers = new AttackStrategy(parameters)
                        .ChooseAttackers()
                        .Concat(ForwardsThatMustAttack)
                        .Distinct()
                        .ToList();

                    if (greedyAttackers.Count > ForwardsThatMustAttack.Count)
                    {
                        // Second choice: attackers selected by greedy attack strategy
                        results.Add(SelectAttackTarget(greedyAttackers));
                    }

                    if (greedyAttackers.Count < everyForwardThatCanAttack.Count)
                    {
                        // Third choice: every forward that can attack
                        results.Add(SelectAttackTarget(everyForwardThatCanAttack));
                    }
                }
            ...
            private ChosenAttackers SelectAttackTarget(List<Card> selectedAttackers)
            {
                ...
                var planeswalker = Planeswalkers.FirstOrDefault();
            ...
            private List<Card> RemoveAttackersIfWeCannotAffordToPayCombatCost(
                List<Card> allAttackers
            )
            {
        ...
        private class ScenarioHandler : Handler
        {
            ...
            protected override void ExecuteQuery()
            {
                Result = GetNextScenarioResult() ?? new ChosenAttackers();
            }
        }
        ...
        private class UiHandler : Handler
        {
            ...
            {
                return ForwardsThatMustAttack.All(forward => attackers.Any(x => x.Card == forward));
            }
            ...
            protected override void ExecuteQuery()
            {
                ...
                while (true)
                {
                    var attackerSpec = new IsValidTargetBuilder()
                        .Is.Card(c =>
                        {
                            return c.CanAttack
                                ...
                                && c.CanPartyAttackWith(result.Attackers.Select(a => a.Card));
                        })
                        .On.Battlefield();
                    ...
                    var selectAttacker = Ui.Dialogs.SelectTarget.Create(
                        new SelectTargetParameters
                        {
                            Validator = validator,
                            CanCancel = false,
                            Instructions = IsValidAttackerDeclaration(result)
                                ? null
                                : "(Additional attackers required.)",
                        }
                    );
                    ...
                    if (result.Any(a => a.Card == attacker))
                    {
                        ...
                        Ui.Publisher.Publish(new AttackerUnselected { Attacker = attacker });
                    ...
                    if (Planeswalkers.Count > 0)
                    {
                        var planeswalkerSpec = new IsValidTargetBuilder()
                            .Is.Card(c =>
                                c.Is().Planeswalker && c.Controller == D.Controller.Opponent
                            )
                            .On.Battlefield();
                        ...
                        var selectPlaneswalker = Ui.Dialogs.SelectTarget.Create(
                            new SelectTargetParameters
                            {
                    ...
                    availableMana -= attacker.CombatCost;
                    Ui.Publisher.Publish(new AttackerSelected { Attacker = attacker });
