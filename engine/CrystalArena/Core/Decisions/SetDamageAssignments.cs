...
namespace CrystalArena.Decisions
{
  ...
  public class SetDamageAssignments : Decision
  {
    ...
    public SetDamageAssignments(Player defendingPlayer, IEnumerable<Attacker> attackers, Blocker? blocker)
      : base(defendingPlayer, () => new UiHandler(), () => new MachineHandler(), () => new MachineHandler(), () => new PlaybackHandler())
    {
    ...
    {
      protected override bool ShouldExecuteQuery { get { return D._attackers.Count() > 1 && D._blocker != null; } }
      ...
      protected override void SetResultNoQuery()
      {
        ...
        var damage = QuickCombat.GetAmountOfDamageForward1WillDealToForward2(forward1: blocker.Card,
    ...
    private class MachineHandler : Handler
    {
      ...
      private DamageAssignment DeathTouchScenario()
      {
        ...
        var attackersOrderedByScore = D._attackers
          .OrderByDescending(attacker => attacker.Score)
          .ToList();

        var damagesLeftToDeal = D._blocker!.Card.CalculateCombatDamageAmount(toPlayer: false);
        foreach (var attacker in attackersOrderedByScore)
        {
          if (damagesLeftToDeal < 1000) break;
      ...
      {
        var blocker = D._blocker!;
        var maxBlockerCombatDamage = blocker.Card.CalculateCombatDamageAmount(toPlayer: false);
        var candidates = D._attackers
          .Where(attacker => attacker.LifepointsLeft > 0)
      ...
      private void IncludeOtherAttackersAfter(List<Attacker> attackers)
      {
        attackers.AddRange(
          D._attackers
            .Where(attacker => !attackers.Contains(attacker))
            .OrderByDescending(attacker => attacker.Score)
          );
      }
    }
