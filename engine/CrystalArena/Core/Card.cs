namespace CrystalArena
{
  ...
  public class Card : GameObject, ITarget, IDamageable, IHashDependancy, IHasColors, IHasLife, IModifiable, IComparable<Card>, IDamageSource
  {
    ...
    public void ReceiveDamage(IDamage damage)
    {
      ...
      if (aggregateDamage != null)
      {
        aggregateDamage.Damages = aggregateDamage.Damages.Where(d => !Game.RedirectDamage(d, this));
      }
      else
      {
    ...
    public void ChangeZone(IZone destination, Action<Card> add)
    {
      ...
      if (destination.Name != source.Name)
      {
        ...
        if (destination.Name == Zone.Hand) JoinedHand.Raise();
        ...
        if (destination.Name == Zone.MainDeck && !_isHidden)
        {
          ...
          else if (source.Name == Zone.Battlefield || source.Name == Zone.BreakZone || source.Name == Zone.RemovedFromPlay)
          {
            Reveal(true);
          }
        }
        ...
        {
          if (source.Name == Zone.Battlefield || source.Name == Zone.BreakZone || source.Name == Zone.RemovedFromPlay)
          {
            Reveal(true);
          }
        }
        ...
        if (source.Name == Zone.Hand) LeftHand.Raise();
      }
    }
    ...
    public int CalculatePreventedDamageAmount(int totalAmount, IDamageSource source, bool isCombat = false)
    {
    ...
    public bool CanBeBlockedBy(Card card)
    {
      ...
      if (Has().UnblockableIfDedenderHasMonsters &&
        card.Controller.Battlefield.Any(x => x.Is().Monster && x.Controller == card.Controller))
      {
        return false;
      }
    ...
    private bool HasProtectionFrom(Effect sourceEffect)
    {
      ...
      if (effectController == Controller) return false;

      return sourceEffect.HasTag(EffectTag.RemoveFromPlay) && HasProtectionFromOpponentRemoveFromPlay();
    }
    ...
    public int CalculateCombatDamageAmount(bool toPlayer, bool singleDamageStep = true, int? powerIncrease = null)
    {
