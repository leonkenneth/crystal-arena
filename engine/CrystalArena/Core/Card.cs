...
namespace CrystalArena
{
            ...
            IDamageSource
    {
        ...
        private readonly TrackableList<ICardModifier> _modifiers =
            new TrackableList<ICardModifier>();
        ...
        public TrackableEvent JoinedHand;
        public TrackableEvent LeftHand;
        ...
        public Card(CardTemplate template)
        {
            ...
            JoinedHand = new TrackableEvent();
            LeftHand = new TrackableEvent();
        }
        ...
        {
            get { return _base.Value.Categories; }
        }

        public List<string> Jobs
        {
            get { return _base.Value.Jobs; }
        }
        ...
        public bool IsAbleToAttack
        {
            get
            {
                return IsPermanent
                    ...
                    && !Has().CannotAttack
                    && (
                        !Has().CanAttackOnlyIfDefenderHasIslands
                        || Controller.Opponent.Battlefield.Any(x => x.Is("island"))
                    )
                    && (CanPartyAttackWith(Combat.Attackers));
            }
        }

        private bool CanPartyAttackWith(IEnumerable<Attacker> attackers)
        {
            return CanPartyAttackWith(attackers.Select(x => x.Card));
        }

        public bool CanPartyAttackWith(IEnumerable<Card> cards)
        {
            if (!cards.Any())
                return true;
            return cards.All(CanPartyAttackWith);
        }

        private bool CanPartyAttackWith(Card card)
        {
            return card.Colors.Any(x => this.Colors.Contains(x));
        }
        ...
        public bool CanTap
        {
            get { return !IsTapped && (!HasSummoningSickness || !Is().Forward || Has().Haste); }
        }
        ...
        {
            get { return Combat.HasBlocker(this); }
        }
        ...
        public int Score
        {
            get
            {
                ...
                switch (Zone)
                {
                    ...
                    case (Zone.Hand):
                        score = ScoreCalculator.CalculateCardInHandScore(
                            this,
                            Game.Ai.IsSearchInProgress
                        );
                        break;

                    case (Zone.BreakZone):
                        score = ScoreCalculator.CalculateCardInBreakZoneScore(this);
                        break;

                    case (Zone.MainDeck):
                        score = ScoreCalculator.CalculateCardInMainDeckScore(this);
                        break;
                }
                ...
                if (
                    ...
                    && Is().Aura
                    && AttachedTo != null
                    && AttachedTo.Controller != Controller
                )
                {
                    return -score;
                }
        ...
        public void ReceiveDamage(IDamage damage)
        {
            ...
            if (!(Is().Forward || Is().Planeswalker) || !IsPermanent)
                return;
        ...
        {
            var amountToPrevent = Game.PreventDamage(
                new PreventDamageParameters
                {
        ...
        public Player GetControllerOfACardThisIsAttachedTo()
        {
            return AttachedTo == null
                ? Controller
                : AttachedTo.GetControllerOfACardThisIsAttachedTo();
        }
            ...
            if (destination.Name != source.Name)
            {
                ...
                if (destination.Name == Zone.MainDeck && !_isHidden)
                {
                    if (source.Name == Zone.Hand)
                    {
                        ...
                        source.Name == Zone.Battlefield
                        || source.Name == Zone.BreakZone
                        || source.Name == Zone.RemovedFromPlay
                    )
                    {
                        Reveal(true);
                    }
        ...
        public Card Initialize(Player owner, Game game)
        {
            ...
            JoinedHand.Initialize(ChangeTracker);
            LeftHand.Initialize(ChangeTracker);
        ...
        public int CalculatePreventedDamageAmount(
            int totalAmount,
        ...
        public bool CanBeBlockedBy(Card card)
        {
            ...
            if (!Has().Flying && card.Has().CanBlockOnlyForwardsWithFlying)
                return false;

            if (Has().CanOnlyBeBlockedByForwardsWithFlying && !card.Has().Flying)
                return false;
            ...
            if (Has().Fear && !card.HasColor(CardColor.Dark) && !card.Is().Artifact)
                return false;
            ...
            if (Has().Mountainwalk && card.Controller.Battlefield.Any(x => x.Is("mountain")))
            {
                return false;
            }

            if (Has().Forestwalk && card.Controller.Battlefield.Any(x => x.Is("forest")))
            {
                return false;
            }

            if (
                Has().UnblockableIfDedenderHasArtifacts
            ...
            if (
                Has().UnblockableIfDedenderHasMonsters
                && card.Controller.Battlefield.Any(x =>
                    x.Is().Monster && x.Controller == card.Controller
                )
            )
            {
                return false;
        ...
        {
            return IsPermanent && !IsTapped && Is().Forward && !Has().CannotBlock;
        }
        ...
        public void Detach(Card card)
        {
            ...
            if (
                ...
                && !card.Controller.Battlefield.Contains(card)
            )
            {
        ...
        public bool HasProtectionFrom(Party party)
        {
            return false;
        }

        public bool HasProtectionFrom(IDamageSource damageSource)
        {
            var party = damageSource as Party;
            var card = damageSource as Card;

            if (party != null)
            {
                return HasProtectionFrom(party);
            }

            if (card != null)
            {
                return HasProtectionFrom(card);
            }

            throw new ArgumentException("Unknown damage source type");
        }
        ...
        public void RemoveFromPlay(Effect? sourceEffect)
        {
            if (!CanBeRemovedFromPlayBy(sourceEffect))
                return;
            Owner.PutCardToRemovedFromPlay(this);
        }

        private bool CanBeRemovedFromPlayBy(Effect? sourceEffect)
        {
            if (sourceEffect == null)
                return true;

            return !HasProtectionFrom(sourceEffect);
        }

        private bool HasProtectionFrom(Effect sourceEffect)
        {
            var effectController = sourceEffect.Controller;
            if (effectController == Controller)
                return false;

            return sourceEffect.HasTag(EffectTag.RemoveFromPlay)
                && HasProtectionFromOpponentRemoveFromPlay();
        }

        private bool HasProtectionFromOpponentRemoveFromPlay()
        {
            return _protections.HasProtectionFrom(EffectTag.RemoveFromPlay);
        }

        public void RemoveFromPlayFrom(Zone from, Effect sourceEffect)
        {
            if (from != Zone)
                return;

            RemoveFromPlay(sourceEffect);
        }
        ...
        public void PutOnTopOfMainDeck()
        {
            Owner.PutCardOnTopOfMainDeck(this);
        }
        ...
        public int CalculateCombatDamageAmount(
            bool toPlayer,
            bool singleDamageStep = true,
            int? powerIncrease = null
        )
        {
            var finalPowerIncrease = powerIncrease ?? GetCombatAbilities().PowerIncrease;
            var total = toPlayer ? 1 : Power.GetValueOrDefault() + finalPowerIncrease;
        ...
        public void PutOnTopOfMainDeckFrom(Zone from)
        {
            if (Zone != from)
                return;
        ...
        {
            if (Is().Backup != other.Is().Backup)
            {
                return Is().Backup.CompareTo(other.Is().Backup);
            }
        ...
        public IEnumerable<Card> Cards => new[] { this };

        public bool HasJob(string job)
        {
            return Jobs.Contains(job.ToLower());
        }
        ...
        public IEffectSource? GetExBurstEffectSource()
        {
            ...
            var abilitiesWithExBurst = _triggeredAbilities.Abilities.Where(ability =>
                ability.HasExBurst
            );
