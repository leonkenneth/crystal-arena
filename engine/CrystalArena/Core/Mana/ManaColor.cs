namespace CrystalArena
{
  ...
  public class ManaColor : IEquatable<ManaColor>
  {
    ...
    public ManaColor(bool isWhite = false, bool isBlue = false, bool isBlack = false, bool isRed = false,
      bool isGreen = false, bool isColorless = false, bool isIce = false, bool isEarth = false, bool isPurple = false,
      bool isPhyrexian = false, bool isCrystal = false)
    {
      IsPhyrexian = isPhyrexian;
      IsCrystal = isCrystal;

      _isColor = new[] {isWhite, isBlue, isBlack, isRed, isGreen, isIce, isEarth, isPurple, isColorless};
    ...
    public bool IsIce { get { return _isColor[5]; } }
    public bool IsEarth { get { return _isColor[6]; } }
    public bool IsPurple { get { return _isColor[7]; } }
    
    public bool IsColorless { get { return _isColor[8]; } }
    ...
    public bool Equals(ManaColor other)
    {
      ...
      return _colorIndices.SequenceEqual(other._colorIndices) 
        && this.IsPhyrexian == other.IsPhyrexian && IsCrystal == other.IsCrystal;
    }
    ...
    public override string ToString()
    {
      ...
      if (IsRed) return "R";
      if (IsGreen) return "G";
      if (IsColorless) return "C";
      if (IsIce) return "I";
      if (IsEarth) return "Y";
      if (IsPurple) return "P";
      if (IsPhyrexian) return "P";
      if (IsMulti) return "M";
      if (IsCrystal) return "Z";
