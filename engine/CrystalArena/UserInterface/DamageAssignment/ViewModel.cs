...
namespace CrystalArena.UserInterface.DamageOrder
{
    ...
    public class ViewModel : ViewModelBase
    {
        ...
        public ViewModel(
            ...
            Decisions.DamageAssignment assignment
        )
        {
            _blocker = blocker;
            _attackers = attackers.ToList();

            _damagesToAssign = blocker.Card.CalculateCombatDamageAmount(
                toPlayer: false,
                powerIncrease: blocker.Card.GetCombatAbilities().PowerIncrease
            );
            _damageAssignments = attackers
                .Select(attacker => new DamageAssignment(attacker))
                .ToList();
        ...
        public Card Blocker
        {
            get { return _blocker.Card; }
        }
        public IEnumerable<DamageAssignment> Damages
        {
            get { return _damageAssignments; }
        }
