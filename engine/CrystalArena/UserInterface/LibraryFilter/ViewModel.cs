namespace CrystalArena.UserInterface.MainDeckFilter
{
    ...
    public class ViewModel : ViewModelBase
    {
        ...
        {
            return new { Type = "MainDeckFilter" };
        }

        public ViewModel(
            IEnumerable<CardInfo> cards,
            Func<CardInfo, object> transformResult,
            Func<CardInfo, int> orderBy
        )
        {
            _orderBy = orderBy;
        ...
        public override void Initialize()
        {
            Task.Factory.StartNew(() =>
            {
                ...
                foreach (
                    var card in cards.OrderBy(x => _orderBy(_cards[x.Name])).ThenBy(x => x.Name)
                )
                {
        ...
        private void UpdateView()
        {
            ...
            Task.Factory.StartNew(() =>
                {
                    var cards = _cardsSearch.Query(Text).Where(x => _cards.ContainsKey(x.Name));
