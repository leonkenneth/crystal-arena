...
namespace CrystalArena.UserInterface.DamageZone
{
    public class ViewModel : ViewModelBase, IDisposable
    {
        ...
        {
            return new { Type = "DamageZone", Cards = Cards.Select(x => x.ToJson()) };
        }
        ...
        {
            foreach (var card in _owner.DamageZone)
            {
                AddCard(card);
            }

            _owner.DamageZone.CardAdded += OnCardAdded;
            _owner.DamageZone.CardRemoved += OnCardRemoved;
        }
