...
namespace CrystalArena.UserInterface.Spell
{
            ...
            IReceive<TargetUnselected>
    {
        ...
        {
            return GetPlayableActivations()
                .Select(playableActivator =>
                {
        ...
        public override void ReceiveMessageType(string type, string jsonMessage)
        {
            switch (type)
            {
                ...
                case "ActivateAbilityFromAbilityId":
                    var message =
                        JsonConvert.DeserializeObject<ActivateAbilityFromAbilityIdMessage>(
                            jsonMessage
                        );
        ...
        public void Receive(UiInteractionChanged message)
        {
            switch (message.State)
            {
                case (InteractionState.PlaySpellsOrAbilities):
                    ...
                    IsPlayable =
                        (Card.CanCast().Count > 0) || (Card.CanActivateAbilities().Count > 0);
                    break;
        ...
        {
            var activation = GetPlayableActivations()
                .Find(activation => activation.Prerequisites.AbilityId == abilityId);
        ...
        private void ActivatePlayableActivator(PlayableActivator activation)
        {
            ...
            var proceed =
                SelectX(activation.Prerequisites, activationParameters)
        ...
        private bool SelectConvokeTargets(ActivationParameters parameters)
        {
            ...
            var spec = new IsValidTargetBuilder()
                .Card(c => c.CanBeTapped && c.Is().Forward && c.Controller == Card.Controller)
                .On.Battlefield();
            ...
            )
            {
                MinCount = 0,
                MaxCount = null,
                Message = "Select forwards to tap for convoke.",
        ...
        private bool SelectDelveTargets(ActivationParameters parameters)
        {
            ...
            var spec = new IsValidTargetBuilder().Is.Card().In.YourBreakZone();
        ...
        )
        {
            ...
            {
                parameters.Targets.Distribution = DistributeAmount(
                    parameters.Targets.Effect,
                    prerequisites.DistributeAmount
                );
            }
