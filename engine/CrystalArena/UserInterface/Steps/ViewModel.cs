namespace CrystalArena.UserInterface.Steps
{
    using System.Collections.Generic;
    using System.Linq;
    using CrystalArena.Infrastructure;
    using Events;
    using Step = CrystalArena.Step;

    public class ViewModel : ViewModelBase, IReceive<TurnStartedEvent>
    {
        private readonly List<UserInterface.Step.ViewModel> _steps =
            new List<UserInterface.Step.ViewModel>();

        public IEnumerable<UserInterface.Step.ViewModel> Steps
        {
            get { return _steps; }
        }
        ...
        {
            return new { Steps = Steps.Select(x => x.ToJson()).ToArray() };
        }
        ...
        private IEnumerable<UserInterface.Step.ViewModel> CreateStepViewModels()
        {
            ...
            yield return ViewModels.Step.Create("Dec. blockers", Step.DeclareBlocker);
            yield return ViewModels.Step.Create(
                "Combat damage",
                Step.CombatDamage,
