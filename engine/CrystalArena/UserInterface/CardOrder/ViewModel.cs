    ...
    public class ViewModel : ViewModelBase
    {
        ...
        public override object ToJson()
        {
            return new
            {
                Type = "CardOrder",
                Title,
                Objects = Objects.Select(x => x.ToJson()),
            };
        }
        ...
        {
            Title = title;
            _objects = objects.Select(x => new OrderedObject(ViewModels, x)).ToList();
        }

        public string Title { get; private set; }
        public int[] Ordering
        {
            get { return _objects.Select(x => x.Order.Value - 1).ToArray(); }
        }
        public virtual bool CanAccept
        {
            get { return _objects.All(x => x.Order.HasValue); }
        }
