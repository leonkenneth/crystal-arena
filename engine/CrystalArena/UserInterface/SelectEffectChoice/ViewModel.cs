...
namespace CrystalArena.UserInterface.SelectEffectChoice
{
    ...
    public class ViewModel : ViewModelBase
    {
        ...
        public ViewModel(IEnumerable<IEffectChoice> choices, string text)
        {
            ...
            Message = text.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries)
                .Select(token =>
                {
                    if (token.StartsWith("#"))
                    {
        ...
        public ChosenOptions ChosenOptions
        {
            get { return new ChosenOptions(_choices.Select(x => x.Selected).ToArray()); }
        }
