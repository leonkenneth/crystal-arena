namespace CrystalArena.CardsMainDeck
{
    ...
    public class NissaWorldwaker : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Nissa, Worldwaker")
                .ManaCost("{3}{G}{G}")
                .Type("Planeswalker Nissa")
                .Text(
                    "{+1}: Target backup you control becomes a 4/4 Elemental forward with trample. It's still a backup.{EOL}"
                ...
                {
                    p.Text =
                        "{+1}: Target backup you control becomes a 4/4 Elemental forward with trample. It's still a backup.";
                    ...
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(c => c.Is().Backup, ControlledBy.SpellOwner).On.Battlefield()
                    );
                    p.TargetingRule(
                        new EffectOrCostRankBy(c => c.Is().Forward || c.IsTapped ? 0 : -c.Score)
                    );
