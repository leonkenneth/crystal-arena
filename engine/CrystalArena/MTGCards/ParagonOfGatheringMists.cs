  ...
  public class ParagonOfGatheringMists : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .Text("Other water forwards you control get +1/+1.{EOL}{U},{T}: Another target water forward you control gains flying until end of turn.")
        .Power(2)
        .Toughness(2)
        .Cast(p =>
        {
        ...
        .ContinuousEffect(p =>
        {
          ...
          p.Selector = (c, ctx) => c.Controller == ctx.You && c.Is().Forward && c.HasColor(CardColor.Water) && c != ctx.Source;
        })
        .ActivatedAbility(p =>
        {
          p.Text = "{U},{T}: Another target water forward you control gains flying until end of turn.";
          ...
          p.TargetSelector.AddEffect(trg => trg
              .Is.Card(c => c.Is().Forward && c.HasColor(CardColor.Water), controlledBy: ControlledBy.SpellOwner, canTargetSelf: false)
              .On.Battlefield());
