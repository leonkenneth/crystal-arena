namespace CrystalArena.CardsMainDeck
{
    ...
    public class InGarruksWake : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("In Garruk's Wake")
                .ManaCost("{7}{B}{B}")
                .Type("Sorcery")
                .Text(
                    "Destroy all forwards you don't control and all planeswalkers you don't control."
                )
                .FlavorText(
                ...
                .Cast(p =>
                {
                    p.Effect = () =>
                        new DestroyAllPermanents(
                            filter: (c, ctx) =>
                                (c.Is().Forward || c.Is("Planeswalker"))
                                && c.Controller == ctx.Opponent
                        );
                });
        }
    }
