namespace CrystalArena.CardsMainDeck
{
    ...
    public class Songstitcher : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Songstitcher")
                .ManaCost("{W}")
                .Type("Forward Human Cleric")
                .Text(
                    "{1}{W}: Prevent all combat damage that would be dealt this turn by target attacking forward with flying."
                )
                ...
                .ActivatedAbility(p =>
                {
                    ...
                    p.Effect = () =>
                        new PreventAllDamageFromSourceUntilEot(preventCombatOnly: true);
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(c => c.Has().Flying && c.IsAttacker).On.Battlefield()
                    );
