namespace CrystalArena.CardsMainDeck
{
    ...
    public class Sporogenesis : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Sporogenesis")
                .ManaCost("{3}{G}")
                .Type("Monster")
                .Text(
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Effect = () =>
                        new ApplyModifiersToTargets(() =>
                            new AddCounters(() => new SimpleCounter(CounterType.Fungus), 1)
                        );
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Effect = () =>
                        new CreateTokens(
                            count: P(e =>
                                e.TriggerMessage<ZoneChangedEvent>()
                                    .Card.CountersCount(CounterType.Fungus)
                            ),
                ...
                {
                    p.Text =
                        "When Sporogenesis leaves the battlefield, remove all fungus counters from all forwards.";
                    p.Trigger(new OnZoneChanged(@from: Zone.Battlefield, to: Zone.BreakZone));
                    p.Effect = () =>
                        new RemoveAllCountersFromPermanents(
                            c => c.Is().Forward,
                            CounterType.Fungus
                        );
                    p.TriggerOnlyIfOwningCardIsInPlay = true;
