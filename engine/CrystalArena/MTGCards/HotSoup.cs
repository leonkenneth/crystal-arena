  ...
  public class HotSoup : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .Text("Equipped forward can't be blocked.{EOL}Whenever equipped forward is dealt damage, destroy it.{EOL}Equip {3}({3}: Attach to target forward you control. Equip only as a sorcery.)")
        .FlavorText("\"Comin' through!\"")
        .ActivatedAbility(p =>
        {
          p.Text = "Equip {3} ({3}: Attach to target forward you control. Equip only as a sorcery.)";
