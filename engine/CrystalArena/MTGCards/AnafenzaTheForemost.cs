namespace CrystalArena.CardsMainDeck
{
    ...
    public class AnafenzaTheForemost : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Anafenza, the Foremost")
                .ManaCost("{W}{B}{G}")
                .Type("Legendary Forward — Human Soldier")
                .Text(
                    "Whenever Anafenza, the Foremost attacks, put a +1/+1 counter on another target tapped forward you control.{EOL}If a forward card would be put into an opponent's breakZone from anywhere, exile it instead."
                )
                .FlavorText(
                ...
                {
                    p.Text =
                        "Whenever Anafenza, the Foremost attacks, put a +1/+1 counter on another target tapped forward you control.";
                ...
                {
                    p.Text =
                        "If a forward card would be put into an opponent's breakZone from anywhere, exile it instead.";
                    p.Trigger(
                        new OnZoneChanged(
                            to: Zone.BreakZone,
