namespace CrystalArena.CardsMainDeck
{
    ...
    public class WasteNot : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Waste Not")
                .ManaCost("{1}{B}")
                .Type("Monster")
                .Text(
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Trigger(
                        new WhenPlayerDiscardsCard(
                            (e, ctx) => ctx.Opponent == e.Player && e.Card.Is().Forward
                        )
                    );
                ...
                {
                    p.Text =
                        "Whenever an opponent discards a backup card, add {B}{B} to your mana pool.";

                    p.Trigger(
                        new WhenPlayerDiscardsCard(
                            (e, ctx) => ctx.Opponent == e.Player && e.Card.Is().Backup
                        )
                    );
                ...
                {
                    p.Text =
                        "Whenever an opponent discards a nonforward, nonland card, draw a card.";

                    p.Trigger(
                        new WhenPlayerDiscardsCard(
                            (e, ctx) =>
                                ctx.Opponent == e.Player
                                && !e.Card.Is().Backup
                                && !e.Card.Is().Forward
                        )
                    );
