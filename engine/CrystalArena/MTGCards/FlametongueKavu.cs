  ...
  public class FlametongueKavu : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{3}{R}")
        .Type("Forward Kavu")
        .Text("When Flametongue Kavu enters the battlefield, it deals 4 damage to target forward.")
        ...
        .Cast(p => p.TimingRule(new WhenOpponentControllsPermanents(
          card => card.Is().Forward && card.Life <= 4 &&
            card.CanBeTargetBySpellsWithColor(CardColor.Fire))))
        .TriggeredAbility(p =>
          {
            p.Text = "When Flametongue Kavu enters the battlefield, it deals 4 damage to target forward.";
