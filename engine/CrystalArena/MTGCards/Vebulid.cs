namespace CrystalArena.CardsMainDeck
{
    ...
    public class Vebulid : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Vebulid")
                .ManaCost("{B}")
                .Type("Forward Horror")
                .Text(
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Trigger(
                        new OnStepStart(Step.EndOfCombat, activeTurn: true, passiveTurn: true)
                        {
                            Condition = ctx =>
                                ctx.Turn.Events.HasAttacked(ctx.OwningCard)
                                || ctx.Turn.Events.HasBlocked(ctx.OwningCard),
                        }
                    );
