namespace CrystalArena.CardsMainDeck
{
    ...
    public class LifesLegacy : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Life's Legacy")
                .ManaCost("{1}{G}")
                .Type("Sorcery")
                .Text(
                ...
                .Cast(p =>
                {
                    p.Cost = new AggregateCost(new PayMana("{1}{G}".Parse()), new Sacrifice());

                    p.TargetSelector.AddCost(
                        trg => trg.Is.Forward(ControlledBy.SpellOwner).On.Battlefield(),
                        trg =>
                        {
                            trg.Message = "Select a forward to sacrifice.";
                        }
                    );

                    p.Effect = () =>
                        new DrawCards(count: P(e => e.Target.Card().Power.GetValueOrDefault()));
