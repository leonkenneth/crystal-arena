namespace CrystalArena.CardsMainDeck
{
    ...
    public class GarrukApexPredator : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Garruk, Apex Predator")
                .ManaCost("{5}{B}{G}")
                .Type("Planeswalker Garruk")
                .Text(
                    "{+1}: Destroy another target planeswalker.{EOL}"
                ...
                .ActivatedAbility(p =>
                {
                    ...
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(c => c.Is().Planeswalker).On.Battlefield()
                    );
                ...
                .ActivatedAbility(p =>
                {
                    ...
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(c => c.Is().Forward).On.Battlefield()
                    );
                    p.TargetingRule(new EffectDestroy());
                    p.TimingRule(new OnFirstMain());
                            ...
                            modifiers: () =>
                            {
                                var cp = new ContinuousEffectParameters
                                {
                                    Modifier = () =>
                                    {
                                        ...
                                        {
                                            Text =
                                                "Whenever this forward attacks, it gets +5/+5 and gains trample until end of turn.",
                                            Effect = () =>
                                    ...
                                    Selector = (card, ctx) =>
                                        card.Controller != ctx.EffectOwner && card.Is().Forward,
                                };
