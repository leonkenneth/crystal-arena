namespace CrystalArena.CardsMainDeck
{
    ...
    public class AcidicSlime : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Acidic Slime")
                .ManaCost("{3}{G}{G}")
                .Type("Forward Ooze")
                .Text(
                ...
                {
                    p.Text =
                        "When Acidic Slime enters the battlefield, destroy target artifact, monster, or backup.";
                    p.Trigger(new OnZoneChanged(to: Zone.Battlefield));
                    p.Effect = () => new DestroyTargetPermanents();
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(card =>
                                card.Is().Artifact || card.Is().Monster || card.Is().Backup
                            )
                            .On.Battlefield()
                    );
