namespace CrystalArena.CardsMainDeck
{
    ...
    public class Momentum : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Momentum")
                .ManaCost("{2}{G}")
                .Type("Monster - Aura")
                .Text(
                    "At the beginning of your upkeep, you may put a growth counter on Momentum.{EOL}Enchanted forward gets +1/+1 for each growth counter on Momentum."
                )
                .Cast(p =>
                {
                    p.Effect = () =>
                        new Attach(() =>
                            new IncreasePowerToughnessEqualToAttachedCounterCount(
                                CounterType.Growth
                            )
                        );
