namespace CrystalArena.CardsMainDeck
{
    ...
    public class HighSentinelsOfArashin : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("High Sentinels of Arashin")
                .ManaCost("{3}{W}")
                .Type("Forward - Bird Soldier")
                .Text(
                    "{Flying}{EOL}High Sentinels of Arashin gets +1/+1 for each other forward you control with a +1/+1 counter on it.{EOL}{3}{W}: Put a +1/+1 counter on target forward."
                )
                .Power(3)
                .Toughness(4)
                ...
                .StaticAbility(p =>
                {
                    p.Modifier(() =>
                        new ModifyPowerToughnessForEachPermanent(
                            ...
                            filter: (c, ctx) =>
                                c.Is().Forward
                                && c.CountersCount(CounterType.PowerToughness) > 0
                                && c != ctx.OwningCard,
