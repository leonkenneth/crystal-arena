namespace CrystalArena.CardsMainDeck
{
    ...
    public class RaidersSpoils : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Raiders' Spoils")
                .ManaCost("{3}{B}")
                .Type("Monster")
                .Text(
                    "Forwards you control get +1/+0.{EOL}Whenever a Warrior you control deals combat damage to a player, you may pay 1 life. If you do, draw a card."
                )
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Trigger(
                        new OnDamageDealt(dmg =>
                            dmg.IsDealtToPlayer && dmg.IsCombat && dmg.Source.IsCategory("warrior")
                        )
                    );

                    p.Effect = () => new PayLifeThen(1, effect: new DrawCards(1));
