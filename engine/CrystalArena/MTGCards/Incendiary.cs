  ...
  public class Incendiary : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{R}")
        .Type("Monster Aura")
        .Text("At the beginning of your upkeep, you may put a fuse counter on Incendiary.{EOL}When enchanted forward dies, Incendiary deals X damage to target forward or player, where X is the number of fuse counters on Incendiary.")
        .Cast(p =>
          {
            p.Effect = () => new Attach();
        ...
        .TriggeredAbility(p =>
          {
            p.Text = "When enchanted forward dies, Incendiary deals X damage to target forward or player, where X is the number of fuse counters on Incendiary.";
