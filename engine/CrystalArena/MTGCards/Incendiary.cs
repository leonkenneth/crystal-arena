namespace CrystalArena.CardsMainDeck
{
    ...
    public class Incendiary : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Incendiary")
                .ManaCost("{R}")
                .Type("Monster Aura")
                .Text(
                    "At the beginning of your upkeep, you may put a fuse counter on Incendiary.{EOL}When enchanted forward dies, Incendiary deals X damage to target forward or player, where X is the number of fuse counters on Incendiary."
                )
                .Cast(p =>
                {
                    p.Effect = () => new Attach();
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Effect = () =>
                        new ApplyModifiersToSelf(() =>
                            new AddCounters(() => new SimpleCounter(CounterType.Fuse), 1)
                        );
                ...
                {
                    p.Text =
                        "When enchanted forward dies, Incendiary deals X damage to target forward or player, where X is the number of fuse counters on Incendiary.";
                    p.Trigger(
                        new OnZoneChanged(
                    ...
                    p.Effect = () =>
                        new DealDamageToTargets(
                            P(e => e.Source.OwningCard.CountersCount(CounterType.Fuse))
                        );
