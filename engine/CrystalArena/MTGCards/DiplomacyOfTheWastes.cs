namespace CrystalArena.CardsMainDeck
{
    ...
    public class DiplomacyOfTheWastes : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Diplomacy of the Wastes")
                .ManaCost("{2}{B}")
                .Type("Sorcery")
                .Text(
                ...
                .FlavorText(
                    "\"Our emissaries are gifted negotiators.\"{EOL}—Alesha, Who Smiles at Death"
                )
                .Cast(p =>
                {
                    p.Effect = () =>
                        new CompoundEffect(
                            new OpponentDiscardsCards(
                                ...
                                filter: card => !card.Is().Backup
                            ),
                            new ChangeLife(-2, P(e => e.Controller.Opponent))
                            {
                                ShouldResolve = ctx =>
                                    ctx.You.Battlefield.Forwards.Any(x => x.Is("warrior")),
                            }
                        );
