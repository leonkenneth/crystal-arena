  ...
  public class WalkingSponge : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .Type("Forward Sponge")
        .ManaCost("{1}{U}")
        .Text("{T}: Target forward loses your choice of flying, first strike, or trample until end of turn.")
        .FlavorText("Not only does it catch fish, it cleans them too.")
        .Power(1)
        .Toughness(1)
        .ActivatedAbility(p =>
          {
            p.Text = "{T}: Target forward loses your choice of flying, first strike, or trample until end of turn.";
            p.Cost = new Tap();
