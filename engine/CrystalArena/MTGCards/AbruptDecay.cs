namespace CrystalArena.CardsMainDeck
{
    ...
    public class AbruptDecay : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Abrupt Decay")
                .ManaCost("{B}{G}")
                .Type("Summon")
                .Text(
                    "Abrupt Decay can't be countered by spells or abilities.{EOL}Destroy target nonland permanent with converted mana cost 3 or less."
                )
                ...
                .Cast(p =>
                {
                    ...
                    p.TargetSelector.AddEffect(trg =>
                        trg.Is.Card(c => !c.Is().Backup && c.ConvertedCost <= 3).On.Battlefield()
                    );
