  ...
  public class AbzanCharm : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{W}{B}{G}")
        .Type("Summon")
        .Text("Choose one —{EOL}• RemoveFromPlay target forward with power 3 or greater.{EOL}• You draw two cards and you lose 2 life.{EOL}• Distribute two +1/+1 counters among one or two target forwards.")
        .Cast(p =>
        {
          p.Text = "{{W}}{{B}}{{G}}: RemoveFromPlay target forward with power 3 or greater.";
          p.Effect = () => new RemoveFromPlayTargets();
          p.TargetSelector.AddEffect(trg => trg.Is.Card(c => c.Is().Forward && c.Power >= 3).On.Battlefield());
        ...
        .Cast(p =>
        {
          p.Text = "{{W}}{{B}}{{G}}: Distribute two +1/+1 counters among one or two target forwards.";
