  ...
  public class BarrageOfBoulders : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{2}{R}")
        .Type("Sorcery")
        .Text("Barrage of Boulders deals 1 damage to each forward you don't control.{EOL}{I}Ferocious{/I} — If you control a forward with power 4 or greater, forwards can't block this turn.")
        .FlavorText("Crude tactics can be effective nonetheless.")
        .Cast(p =>
        {
          p.Effect = () => new FerociousEffect(
            L(new DealDamageToForwardsAndPlayers(
                amountForward: 1,
                filterForward: (effect, card) => card.Controller != effect.Source.OwningCard.Controller)),
