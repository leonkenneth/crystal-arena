namespace CrystalArena.CardsMainDeck
{
    ...
    public class NightshadeSeer : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Nightshade Seer")
                .ManaCost("{3}{B}")
                ...
                .ActivatedAbility(p =>
                {
                    ...
                    p.TimingRule(
                        new WhenYourHandCountIs(
                            minCount: 1,
                            selector: c => c.HasColor(CardColor.Dark)
                        )
                    );
                    p.TargetingRule(
                        new EffectReduceToughness(getAmount: tp =>
                            tp.Controller.Hand.Count(c => c.HasColor(CardColor.Dark))
                        )
                    );
