  ...
  public class PlanarVoid : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{B}")
        .Type("Monster")
        .Text("Whenever another card is put into a breakZone from anywhere, exile that card.")
        .FlavorText("'Planeswalking isn't about walking. It's about falling and screaming.'")
        .Cast(p => p.TimingRule(new OnFirstMain()))
        .TriggeredAbility(p =>
          {
            p.Text = "Whenever another card is put into a breakZone from anywhere, exile that card.";
