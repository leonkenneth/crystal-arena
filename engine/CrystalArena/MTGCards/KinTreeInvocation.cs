  ...
  public class KinTreeInvocation : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{B}{G}")
        .Type("Sorcery")
        .Text("Put an X/X dark and wind Spirit Warrior forward token onto the battlefield, where X is the greatest toughness among forwards you control.")
        ...
        .Cast(p =>
        {
          p.Effect = () => new CreateTokens(
            ...
            {
              token.Power(ctx.You.Battlefield.Forwards.Max(c => c.Toughness.GetValueOrDefault()));
              token.Toughness(ctx.You.Battlefield.Forwards.Max(c => c.Toughness.GetValueOrDefault()));
            });
