namespace CrystalArena.CardsMainDeck
{
    ...
    public class PhyrexianPlaguelord : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Phyrexian Plaguelord")
                .ManaCost("{3}{B}{B}")
                ...
                {
                    p.Text =
                        "{T}, Sacrifice Phyrexian Plaguelord: Target forward gets -4/-4 until end of turn.";
                ...
                .ActivatedAbility(p =>
                {
                    ...
                    p.TimingRule(
                        new Any(new BeforeYouDeclareAttackers(), new WhenStackIsNotEmpty())
                    );
                    p.TargetingRule(
                        new CostSacrificeEffectReduceToughness(1) { ConsiderTargetingSelf = false }
                    );
                });
        }
    }
