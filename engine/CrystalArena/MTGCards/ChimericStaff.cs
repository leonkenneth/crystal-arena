  ...
  public class ChimericStaff : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{4}")
        .Type("Artifact")
        .Text("{X}: Chimeric Staff becomes an X/X Construct artifact forward until end of turn.")
        .FlavorText("A snake in the grasp.")
        .ActivatedAbility(p =>
          {
            p.Text = "{X}: Chimeric Staff becomes an X/X Construct artifact forward until end of turn.";
            p.Cost = new PayMana(Mana.Zero, hasX: true);

            p.Effect = () => new ApplyModifiersToSelf(() =>
              new ChangeToForward(
                power: Value.PlusX, toughness: Value.PlusX,
                type: t => t.Add(baseTypes: "artifact forward", subTypes: "construct")) {UntilEot = true});
