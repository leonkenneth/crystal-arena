  ...
  public class HiddenStag : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .TriggeredAbility(p =>
          {
            p.Text = "Whenever you play a backup, if Hidden Stag is a forward, Hidden Stag becomes an monster.";
            p.Trigger(new OnBackupPlayed(
              filter: (ability, card) =>
                ability.OwningCard.Controller == card.Controller && ability.OwningCard.Is().Forward && card.Is().Backup));
