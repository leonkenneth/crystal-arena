namespace CrystalArena.CardsMainDeck
{
    ...
    public class MasterOfPredicaments : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Master of Predicaments")
                .ManaCost("{3}{U}{U}")
                .Type("Forward — Sphinx")
                .Text(
                    "{Flying}{EOL}Whenever Master of Predicaments deals combat damage to a player, choose a card in your hand. That player guesses whether the card's converted mana cost is greater than 4. If the player guessed wrong, you may cast the card without paying its mana cost."
                )
                .Power(4)
                .Toughness(4)
                ...
                .TriggeredAbility(p =>
                {
                    ...
                    p.Trigger(
                        new OnDamageDealt(dmg =>
                            dmg.IsDealtByOwningCard && dmg.IsCombat && dmg.IsDealtToPlayer
                        )
                    );

                    p.Effect = () =>
                        new CastCardWithoutpayingIfOpponentGuessedWrong(
