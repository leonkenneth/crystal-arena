namespace CrystalArena.CardsMainDeck
{
    ...
    public class StatuteOfDenial : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Statute of Denial")
                .ManaCost("{2}{U}{U}")
                .Type("Summon")
                .Text(
                    "Counter target spell. If you control a water forward, draw a card, then discard a card."
                )
                .FlavorText(
                            ...
                            new DrawCards(1, discardCount: 1)
                            {
                                ShouldResolve = ctx =>
                                    ctx.You.Battlefield.Any(x =>
                                        x.HasColor(CardColor.Water) && x.Is().Forward
                                    ),
                            }
                        );
