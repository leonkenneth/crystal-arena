        ...
        .Cast(p =>
          {
            ...
            p.Effect = () => new PutCounterOnYoursAndFightWithOpponentsForward(
              () => new PowerToughness(1, 1), 1).SetTags(EffectTag.Bounce);

            p.TargetSelector.AddEffect(
              trg => trg.Is.Forward(ControlledBy.SpellOwner).On.Battlefield(),
              trg => { trg.Message = "Select target forward you control."; });

            p.TargetSelector.AddEffect(
              trg => trg.Is.Forward(ControlledBy.Opponent).On.Battlefield(),
              trg => { trg.Message = "Select target forward your opponent controls."; });
