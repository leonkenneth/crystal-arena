namespace CrystalArena.CardsMainDeck
{
    ...
    public class LeechingSliver : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Leeching Sliver")
                .ManaCost("{1}{B}")
                .Type("Forward — Sliver")
                .Text("Whenever a Sliver you control attacks, defending player loses 1 life.")
                .FlavorText(
                    "\"Seeing one slowly devour one of my party begged the question—do they know what cruelty is?\"{EOL}—Hastric, Thunian scout"
                )
                .Power(1)
                .Toughness(1)
                .TriggeredAbility(p =>
                {
                    ...
                    p.Trigger(
                        new WhenAForwardAttacks(t =>
                            t.Opponent && t.AttackerHas(c => c.Is("sliver"))
                        )
                    );
