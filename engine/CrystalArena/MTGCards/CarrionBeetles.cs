  ...
  public class CarrionBeetles : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{B}")
        .Type("Forward Insect")
        .Text("{2}{B},{T}: RemoveFromPlay up to three target cards from a single breakZone.")
        .FlavorText("It's all fun and games until someone loses an eye.")
        .Power(1)
        .Toughness(1)
        .ActivatedAbility(p =>
          {
            p.Text = "{2}{B},{T}: RemoveFromPlay up to three target cards from a single breakZone.";
