namespace CrystalArena.CardsMainDeck
{
    ...
    public class ShowAndTell : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Show and Tell")
                .ManaCost("{2}{U}")
                .Type("Sorcery")
                .Text(
                ...
                {
                    p.Effect = () =>
                        new EachPlayerPutsACardToBattlefield(
                            zone: Zone.Hand,
                            filter: c =>
                                c.Is().Forward || c.Is().Artifact || c.Is().Monster || c.Is().Backup
                        );
                    ...
                    p.TimingRule(
                        new WhenYourHandCountIs(
                            ...
                            selector: c =>
                                c.ConvertedCost >= 6
                                && (c.Is().Forward || c.Is().Artifact || c.Is().Monster)
                        )
                    );
                });
        }
