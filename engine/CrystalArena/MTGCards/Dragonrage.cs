namespace CrystalArena.CardsMainDeck
{
    ...
    public class Dragonrage : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Dragonrage")
                .ManaCost("{2}{R}")
                .Type("Summon")
                .Text(
                    "Add {R} to your mana pool for each attacking forward you control. Until end of turn, attacking forwards you control gain \"{R}: This forward gets +1/+0 until end of turn.\""
                )
                .FlavorText(
                    "\"Dragons in the skies of Tarkir—for the first time, it feels like my home.\"{EOL}—Sarkhan Vol"
                )
                .Cast(p =>
                {
                    p.Effect = () =>
                        new CompoundEffect(
                            new AddManaToPool(
                                P(e =>
                                    Mana.Colored(
                                        ManaColor.Fire,
                                        e.Controller.Battlefield.Forwards.Count(x => x.IsAttacker)
                                    )
                                )
                            ),
                            new ApplyModifiersToPermanents(
                                selector: (c, ctx) =>
                                    c.Is().Forward && c.IsAttacker && ctx.You == c.Controller,
                                modifier: () =>
                                {
