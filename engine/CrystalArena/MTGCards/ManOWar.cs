namespace CrystalArena.CardsMainDeck
{
    ...
    public class ManOWar : CardTemplateSource
    {
        ...
        {
            yield return Card.Named("Man-o'-War")
                .ManaCost("{2}{U}")
                .Type("Forward Jellyfish")
                .Text(
                    "When Man-o'-War enters the battlefield, return target forward to its owner's hand."
                )
                ...
                .Cast(p =>
                {
                    p.TimingRule(
                        new WhenOpponentControllsPermanents(card =>
                            card.Is().Forward && card.CanBeTargetBySpellsWithColor(CardColor.Water)
                        )
                    );
