        ...
        .ActivatedAbility(p =>
          {
            ...
            p.Effect = () => new ApplyModifiersToTargets(() => new ChangeToForward(
              power: m => m.OwningCard.ConvertedCost,
              toughness: m => m.OwningCard.ConvertedCost,
              type: m => m.OwningCard.Type.Add(baseTypes: "forward")) {UntilEot = true});

            p.TargetSelector.AddEffect(trg => trg
              .Is.Card(c => c.Is().Artifact && !c.Is().Forward)
              .On.Battlefield());
