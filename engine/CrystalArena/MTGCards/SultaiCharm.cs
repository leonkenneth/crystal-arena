  ...
  public class SultaiCharm : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{B}{G}{U}")
        .Type("Summon")
        .Text("Choose one —{EOL}• Destroy target monocolored forward.{EOL}• Destroy target artifact or monster.{EOL}• Draw two cards, then discard a card.")
        .Cast(p =>
        {
          p.Text = "{{B}}{{G}}{{U}}: Destroy target monocolored forward.";
          p.Effect = () => new DestroyTargetPermanents();
          p.TargetSelector.AddEffect(trg => trg.Is.Card(c => c.Is().Forward && !c.IsMultiColored).On.Battlefield());
