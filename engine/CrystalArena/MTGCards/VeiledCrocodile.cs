  ...
  public class VeiledCrocodile : CardTemplateSource
  {
    public override IEnumerable<CardTemplate> GetCards()
    {
      yield return Card
        ...
        .ManaCost("{2}{U}")
        .Type("Monster")
        .Text("When a player has no cards in hand, if Veiled Crocodile is an monster, Veiled Crocodile becomes a 4/4 Crocodile forward.")
        .FlavorText("Some roads are paved with bad intentions.")
        .Cast(p => p.TimingRule(new OnFirstMain()))
        .TriggeredAbility(p =>
          {
            p.Text = "When a player has no cards in hand, if Veiled Crocodile is an monster, Veiled Crocodile becomes a 4/4 Crocodile forward.";
