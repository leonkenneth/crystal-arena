namespace CrystalArena.CardsMainDeck
{
  ...
  public class NoRestForTheWicked : CardTemplateSource
  {
    ...
    {
      return card.Is().Forward && game.Turn.Events.HasChangedZone(card, @from: Zone.Battlefield, to: Zone.BreakZone);
    }
        ...
        .ActivatedAbility(p =>
          {
            ...
            p.Effect = () => new ReturnAllCardsInBreakZoneToHand(WasPutIntoBreakZoneThisTurnFromBattlefield);

            p.TimingRule(new Any(new OnStep(Step.EndOfTurn), new WhenOwningCardWillBeDestroyed()));
            p.TimingRule(new WhenYourBreakZoneCountIs(minCount: 1,
