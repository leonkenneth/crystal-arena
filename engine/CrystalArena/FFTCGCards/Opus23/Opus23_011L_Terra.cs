        ...
        yield return Card
            ...
            .TriggeredAbility(p =>
            {
                ...
                p.Effect = () => new CompoundEffect(
                    new ChooseInBreakZonePutToZone(Zone.Hand, validator: (card, ctx) => card.Is().Summon),
                    new ApplyModifiersToGame(() =>
                        new AddCostModifier(new ChangeManaCostOfSpellsOrAbilities(2,
                                (card, costType, modifier) => card.Is().Summon && costType == CostType.Spell))
                            { UntilEot = true }));
            })
        .ContinuousEffect(p =>
           {
