            ...
            .TriggeredAbility(p =>
                {
                    ...
                    p.Effect = () =>
                        new CompoundEffect(new SearchMainDeckPutToZone(
                            zone: Zone.Hand,
                            minCount: 0,
                            maxCount: 1,
                            validator: (c, ctx) => c.Name == "Bahamut",
                            text: "Search for 1 Card Name Bahamut."), new ApplyModifiersToGame(() =>
                            new AddCostModifier(new ChangeManaCostOfSpellsOrAbilities(-5,
                                (card, costType, modifier) => card.Name == "Bahamut" && costType == CostType.Spell))));
                }
            );
    }
}