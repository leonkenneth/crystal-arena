...
namespace CrystalArena.Infrastructure
{
    ...
    public class ChangeTracker : ICopyable, INotifyChangeTracker
    {
        ...
        private static readonly ThreadLocal<bool?> _isEnabledThreadLocal = new ThreadLocal<bool?>();
        ...
        public void NotifyValueWillBeRemoved<T>(
            ITrackableCollection<T> trackableCollection,
            T removed
        )
        {
            AssertNotLocked();
            ...
            if (list != null)
            {
                ...
                _changeHistory.Push(() =>
                    trackableCollection.InsertWithoutTracking(removed, index)
                );
        ...
        public class Guard : INotifyChangeTracker
        {
            ...
            public void NotifyCollectionWillBeCleared<T>(
                ITrackableCollection<T> trackableCollection
            )
            {
                Fail();
            ...
            {
                _isEnabledThreadLocal.Value = true;
            }

            public static void Disable()
            {
                _isEnabledThreadLocal.Value = false;
            }

            private static void Fail()
            {
                var enable = (bool?)_isEnabledThreadLocal.Value;
