namespace CrystalArena.Infrastructure
{
    ...
    public class TrackableList<T> : ITrackableCollection<T>, IList<T>, IHashable
    {
        ...
        public TrackableList<T> Initialize(
            INotifyChangeTracker changeTracker,
            IHashDependancy hashDependancy = null
        )
        {
        ...
        {
            var head = _items.Take(permutation.Length).ToList();
        ...
        {
            var tail = _items.Skip(_items.Count - permutation.Length).ToList();
