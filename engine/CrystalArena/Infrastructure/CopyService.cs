namespace CrystalArena.Infrastructure
{
    ...
    public class CopyService
    {
        ...
        private object CopyCopyable(object original)
        {
            ...
            if (contributor != null)
            {
                _pendingContributors.Add(() => contributor.AfterMemberCopy(original));
            }
                        ...
                        baseType
                            ...
                            .Where(x =>
                                !(
                                    ...
                                    && x.FieldType.GetGenericTypeDefinition()
                                        == typeof(EventHandler<>)
                                )
                            )
        ...
        private class TypeCache
        {
            ...
            private readonly Dictionary<Type, ParameterlessCtor> _ctors =
                new Dictionary<Type, ParameterlessCtor>();
            private readonly Dictionary<Type, CopyHandler> _handlers =
                new Dictionary<Type, CopyHandler>();
                ...
                lock (_ctorLock)
                {
                    ...
                    if (_ctors.TryGetValue(type, out ctor) == false)
                    {
                        ...
                        Asrt.True(
                            ctor != null,
                            String.Format(
                                "Type {0} is marked with [Copyable] but is missing a parameterless constructor.",
                                type
                            )
                        );
