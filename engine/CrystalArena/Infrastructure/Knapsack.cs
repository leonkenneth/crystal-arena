        ...
        public static List<KnapsackItem<T>> Solve<T>(IList<KnapsackItem<T>> items, int maxWeight)
        {
            ...
            for (var curMaxWeight = 1; curMaxWeight <= maxWeight; curMaxWeight++)
            {
                for (var i = 0; i < items.Count; i++)
                {
                    ...
                    var scoreIncludingCurrent = new Lazy<int>(() =>
                        scores[i, curMaxWeight - items[i].Weight] + items[i].Value
                    );

                    if (
                        items[i].Weight <= curMaxWeight
                        && scoreUpToCurrent < scoreIncludingCurrent.Value
                    )
                    {
