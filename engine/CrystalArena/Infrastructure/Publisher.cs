namespace CrystalArena.Infrastructure
{
    ...
    public class Publisher : ICopyable
    {
        ...
        private Dictionary<Type, TrackableList<object>> _subscribersByEvent =
            new Dictionary<Type, TrackableList<object>>();

        public Publisher(
            Assembly assembly = null,
            INotifyChangeTracker changeTracker = null,
            string ns = null
        )
        {
        ...
        public void Copy(object original, CopyService copyService)
        {
            ...
            foreach (var subscriber in org._subscribersByEvent)
            {
                var subscribers = subscriber
                    .Value.Where(x => IsUiComponent(x) == false)
        ...
        private class EventsCache
        {
            ...
            {
                var typesAndTheirEvents = assembly
                    .GetTypes()
                    .Where(type =>
                        (@namespace == null || Equals(type.Namespace, @namespace))
                        && type.Implements<IReceive>()
                    )
                    .Select(type => new { Type = type, Events = GetEventsForType(type).ToList() });
            ...
            private IEnumerable<Type> GetEventsForType(Type type)
            {
                return type.GetInterfaces()
                    .Where(x =>
                        x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IReceive<>)
                    )
