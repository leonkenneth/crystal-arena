    ...
    public class Combat
    {
        ...
        public class Predefined : PredefinedScenario
        {
            ...
            public void PartyAttackDealsOnly1PointToPlayer()
            {
                ...
                Exec(
                    At(Step.DeclareAttackers).DeclareAttackers(fwd1, fwd2),
                    At(Step.SecondMain).Verify(() => Equal(6, P2.Life))
                );
            }
            ...
            public void DestroyAttacker()
            {
                ...
                Exec(
                    At(Step.DeclareAttackers)
                        ...
                        .Cast(shock, bear)
                        .Verify(() => True(Combat.Attackers.None()))
                );
            }
            ...
            public void SummonExBurst()
            {
                ...
                Exec(
                    // P1 attacks
                    At(Step.DeclareAttackers, turn: 1).DeclareAttackers(attacker),
                    // P2 chooses to use ex-burst when card goes to DamageZone
                    At(Step.CombatDamage, turn: 1)
                        .Answer(true)
                        .Cast(exBurstSummon, target: attacker),
                    // Attacker is killed
                    At(Step.SecondMain, turn: 1).Verify(() => Equal(0, P1.Battlefield.Count()))
                );
            }
            ...
            public void TriggeredAbilityExBurst()
            {
                ...
                Exec(
                    ...
                    At(Step.DeclareAttackers, turn: 1).DeclareAttackers(attacker),
                    // P2 chooses to use ex-burst when card goes to DamageZone
                    At(Step.CombatDamage, turn: 1).Answer(true).Target(attacker),
                    // Attacker is killed
                    At(Step.SecondMain, turn: 1).Verify(() => Equal(0, P1.Battlefield.Count()))
                );
            }
