    ...
    public class Endoskeleton
    {
        public class PredefinedAi : PredefinedAiScenario
        {
            ...
            public void LeaveTapped()
            {
                ...
                Exec(
                    At(Step.DeclareAttackers).DeclareAttackers(knight),
                    At(Step.SecondMain)
                        .Verify(() =>
                        {
                            Equal(5, C(kavu).Toughness);
                            Equal(Zone.BreakZone, C(knight).Zone);
                        }),
                    At(Step.Upkeep, 2)
                        .Verify(() =>
                        {
            ...
            //[Fact (Skip = "Old card")] todo this does not work because ai considers only single blocking declaration and forward pumping is considered only for attackers and blockers
            public void PumpAutomation()
            {
                ...
                Exec(
                    At(Step.DeclareAttackers).DeclareAttackers(treefolk),
                    At(Step.SecondMain).Verify(() => Equal(5, C(automation).Toughness))
                );
            }
        }
    }
