    ...
    public class Somnophore
    {
        ...
        public class PredefinedAi : PredefinedAiScenario
        {
            ...
            public void UntapNormalyIfDestroyed()
            {
                ...
                Exec(
                    At(Step.SecondMain, turn: 2).Verify(() => True(C(engine).IsTapped)),
                    At(Step.EndOfTurn, turn: 2).Cast(shock, target: somnophore),
                    At(Step.Upkeep, turn: 3).Verify(() => False(C(engine).IsTapped))
                );
            }
        }
    }
