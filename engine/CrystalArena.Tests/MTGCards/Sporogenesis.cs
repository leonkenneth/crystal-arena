    ...
    public class Sporogenesis
    {
        ...
        public class Predefined : PredefinedScenario
        {
            ...
            public void PutASaprolingInPlayForEachCounter()
            {
                ...
                Exec(
                    At(Step.Upkeep, turn: 1).Target(bear),
                    At(Step.Upkeep, turn: 3).Target(bear),
                    At(Step.FirstMain, turn: 3).Cast(shock, target: bear),
            ...
            public void RemoveAllCounters()
            {
                ...
                Exec(
                    At(Step.Upkeep, turn: 1).Target(bear),
                    At(Step.Upkeep, turn: 3).Target(bear),
                    At(Step.FirstMain, turn: 3).Cast(disenchant, target: sporogenesis),
                    At(Step.SecondMain, turn: 3).Verify(() => Equal(0, C(bear).Counters))
                );
            }
        }
    }
