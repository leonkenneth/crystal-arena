namespace CrystalArena.Tests.Cards
{
    ...
    public class Pariah
    {
        ...
        {
            [Fact(Skip = "Old card")]
            public void RedirectToKillOpponentForward()
            {
                ...
                Exec(
                    At(Step.SecondMain).Cast(pariah, baloth),
                    At(Step.DeclareAttackers, turn: 2).DeclareAttackers(baloth),
                    At(Step.SecondMain, turn: 2)
                        .Verify(() =>
                        {
                            Equal(20, P1.Life);
                            Equal(Zone.BreakZone, C(baloth).Zone);
                            Equal(Zone.BreakZone, C(pariah).Zone);
                        })
                );
            }
        }
