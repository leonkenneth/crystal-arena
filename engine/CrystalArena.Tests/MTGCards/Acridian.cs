    ...
    public class Acridian
    {
        ...
        public class Predefined : PredefinedScenario
        {
            ...
            public void EchoAfterReanimate()
            {
                ...
                Exec(
                    At(Step.FirstMain, turn: 1).Cast(acridian),
                    At(Step.FirstMain, turn: 3)
                        .Cast(beacon, target: acridian)
                        .Verify(() =>
                        {
                            Equal(Zone.BreakZone, C(acridian).Zone);
                        }),
                    At(Step.SecondMain, turn: 3).Cast(unearth, target: acridian),
                    At(Step.FirstMain, turn: 5)
                        .Verify(() =>
                        {
                            Equal(Zone.Battlefield, C(acridian).Zone);
                            Equal(2, P1.Battlefield.Count(x => x.Is().Backup && x.IsTapped));
                        })
                );
            }
        }
