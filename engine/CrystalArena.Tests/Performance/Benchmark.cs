namespace CrystalArena.Tests.Performance
{
    ...
    public class Benchmark
    {
        ...
        public static BenchmarkResult RunAndCompare(
            BenchmarkRunConfiguration run,
            BenchmarkRunConfiguration compareTo
        )
        {
            Asrt.True(
                File.Exists(compareTo.GetBenchmarkBaseFilename()),
        ...
        private static BenchmarkRunResult Run(BenchmarkRunConfiguration configuration)
        {
            ...
            foreach (var methodInfo in tests)
            {
                ...
                runs.Add(new ScenarioRunResult(methodInfo.Name, stopwatch.Elapsed));
