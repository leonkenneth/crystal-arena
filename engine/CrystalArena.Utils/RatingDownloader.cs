namespace CrystalArena.Utils
{
    ...
    public class RatingDownloader
    {
        private static readonly Regex RatingPattern = new Regex(
            @"textRatingValue.*>(\d\.\d*)</span>",
            RegexOptions.Compiled
        );
        private static readonly Regex RarityPattern = new Regex(
            @">(Common|Rare|Uncommon)</span></div>",
            RegexOptions.Compiled
        );
        ...
        public Decimal? TryDownloadRating(string cardName, out string rarity)
        {
            ...
            var url = String.Format(
                @"http://gatherer.wizards.com/Pages/Search/Default.aspx?name={0}",
                encodedCardName
            );
