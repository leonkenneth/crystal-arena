namespace CrystalArena.Utils
{
    ...
    public class GenerateDecks : Task
    {
        public override bool Execute(Arguments arguments)
        {
            ...
            {
                starterSet = MediaMainDeck.RandomSet();
                boosterSet1 = MediaMainDeck.RandomSet();
                boosterSet2 = MediaMainDeck.RandomSet();
                boosterSet3 = MediaMainDeck.RandomSet();
            }
            else
            {
                starterSet = MediaMainDeck.GetSet(arguments["s"]);
                boosterSet1 = MediaMainDeck.GetSet(arguments["b1"]);
                boosterSet2 = MediaMainDeck.GetSet(arguments["b2"]);
                boosterSet3 = MediaMainDeck.GetSet(arguments["b3"]);
            }
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var ratings = CardRatings.Merge(
                    CardRatings.Merge(boosterSet2.Ratings, boosterSet3.Ratings),
                    CardRatings.Merge(starterSet.Ratings, boosterSet1.Ratings)
                );

                var deck = DeckBuilder.BuildDeck(library, ratings);
                deck.LimitedCode = MagicSet.GetLimitedCode(
                    starterSet.Name,
