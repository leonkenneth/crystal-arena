...
// Get emoji for card
function getCardEmoji(label: string): string {
...
function PreviewCardOverlay<T>({ renderCard }: { renderCard: (card: T, faceDown: boolean) => React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {getCardEmoji(previewCard.label)}
        </div>
...
function SelectedCardOverlay<T>({ renderCard }: { renderCard: (card: T, faceDown: boolean) => React.ReactNode }) {
      ...
      >
        ...
        <div
          style={{
            ...
            boxShadow: '0 0 30px rgba(255,215,0,0.5), 0 10px 40px rgba(0,0,0,0.5)',
            display: 'flex',
            flexDirection: 'column',
            padding: '12px',
            color: '#333',
        ...
        >
          ...
          >
            {getCardEmoji(selectedCard.label)}
          </div>
...
function ExpandedPileOverlay<T>({ renderCard }: { renderCard: (card: T, faceDown: boolean) => React.ReactNode }) {
        ...
        >
          {expandedPile.cards.map((card, index) => (
            ...
            >
              ...
              >
                {isDeck ? '🎴' : getCardEmoji(card.label)}
              </div>

              {/* Stats (only for graveyard) */}
              {!isDeck && (card.attack !== undefined || card.health !== undefined) && (
                <div
                  style={{
                    display: 'flex',
                    justifyContent: 'space-between',
...
function GameBoardCanvas<T>(props: GameBoardProps<T>) {
  return (
    <>
      ...
      {/* Card overlays */}
      <PreviewCardOverlay renderCard={props.renderCard} />
      <SelectedCardOverlay renderCard={props.renderCard} />
      <ExpandedPileOverlay renderCard={props.renderCard} />
    </>
  )
}
